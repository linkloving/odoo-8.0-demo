...
class BaseModel(object):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _field_create(self, cr, context=None):
        ...
        for (k, f) in model_fields:
            ...
            if getattr(f, 'serialization_field', None):
                ...
                serialization_field_id = ir_model_fields_obj.search(cr, SUPERUSER_ID, [('model', '=', vals['model']), (
                ...
                for key, val in vals.items():
                    if cols[k][key] != vals[key]:
                        ...
                        WHERE
                            ...
                            bool(vals['selectable']), vals['relation_field'], bool(vals['translate']),
                            vals['serialization_field_id'], vals['model'], vals['name']
                        ))
                        break
    ...
    def import_data(self, cr, uid, fields, datas, mode='init', current_module='', noupdate=False, context=None,
                    filename=None):
        """
        .. deprecated:: 7.0
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        if view_id:
            ...
            root_view = View.read_combined(cr, uid, view_id,
                                           fields=['id', 'name', 'field_parent', 'type', 'model', 'arch'],
                                           context=context)
        ...
        xarch, xfields = View.postprocess_and_fields(cr, uid, self._name, etree.fromstring(result['arch']), view_id,
                                                     context=ctx)
        result['arch'] = xarch
        ...
        if toolbar:
            toclean = (
            'report_sxw_content', 'report_rml_content', 'report_sxw', 'report_rml', 'report_sxw_content_data',
            ...
            resaction = ir_values_obj.get(cr, uid, 'action', 'client_action_multi', [(self._name, False)], False,
                                          context)
            resrelate = ir_values_obj.get(cr, uid, 'action', 'client_action_relate', [(self._name, False)], False,
                                          context)
        ...
        for lang in langs:
            ...
            for f in fields:
                if f in self._columns:
                    res_trans = self.pool.get('ir.translation')._get_source(cr, uid, self._name + ',' + f, 'field',
                                                                            lang)
                    if res_trans:
        ...
        for lang in langs:
            for field in vals:
                if field in self._columns:
                    ...
                    self.pool.get('ir.translation')._set_ids(cr, uid, self._name + ',' + field, 'field', lang, [0],
                                                             vals[field], src)
        for table in self._inherits:
        ...
        if missing_defaults:
            ...
            for dv in defaults:
                ...
                if (dv in self._columns and self._columns[dv]._type == 'one2many' \
                        ...
                        and isinstance(defaults[dv], (list, tuple)) and defaults[dv] and isinstance(defaults[dv][0],
                                                                                                    dict):
    ...
    def _read_group_format_result(self, data, annotated_groupbys, groupby, groupby_dict, domain, context):
        ...
        """
        domain_group = [dom for gb in annotated_groupbys for dom in
                        self._read_group_get_domain(gb, data[gb['groupby']])]
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False,
        ...
        self._fields[f].group_operator or 'sum', self._inherits_join_calc(self._table, f, query), f)
    ...
    def _check_removed_columns(self, cr, log=False):
        ...
        columns = [c for c in self._columns if
                   not (isinstance(self._columns[c], fields.function) and not self._columns[c].store)]
                        ...
                        if f_obj_type:
                            ...
                            if f_pg_type == 'varchar' and f._type in ('char', 'selection') and f_pg_size and (
                                ...
                                try:
                                    ...
                                    cr.execute(
                                        'ALTER TABLE "%s" RENAME COLUMN "%s" TO temp_change_size' % (self._table, k))
                                    cr.execute(
                                        'ALTER TABLE "%s" ADD COLUMN "%s" %s' % (self._table, k, pg_varchar(f.size)))
    ...
    def _select_column_data(self, cr):
        ...
        cr.execute(
            "SELECT c.relname,a.attname,a.attlen,a.atttypmod,a.attnotnull,a.atthasdef,t.typname,CASE WHEN a.attlen=-1 THEN (CASE WHEN a.atttypmod=-1 THEN 0 ELSE a.atttypmod-4 END) ELSE a.attlen END as size " \
        ...
        if f._obj in self.pool:
            ...
            if f._fields_id not in other._columns.keys():
                if f._fields_id not in other._inherit_fields.keys():
                    raise except_orm('Programming Error',
                                     "There is no reference field '%s' found for '%s'" % (f._fields_id, f._obj,))
    ...
    def _add_sql_constraints(self, cr):
        ...
        for (key, con, _) in self._sql_constraints:
            ...
            cr.execute(
                "SELECT conname, pg_catalog.pg_get_constraintdef(oid, true) as condef FROM pg_constraint where conname=%s",
                (conname,))
            ...
            if not existing_constraints:
                ...
                sql_actions['drop']['msg_ok'] = sql_actions['drop']['msg_ok'] % (
                existing_constraints[0]['condef'].lower(),)
                sql_actions['add']['execute'] = True
                sql_actions['add']['msg_err'] = sql_actions['add']['msg_err'] % (sql_actions['add']['query'],)
    ...
    def _check_concurrency(self, cr, ids, context):
        ...
        for sub_ids in cr.split_for_in_conditions(ids):
            ...
            if not ids_to_check:
                continue
            cr.execute(
                "SELECT id FROM %s WHERE %s" % (self._table, " OR ".join([check_clause] * (len(ids_to_check) / 2))),
                tuple(ids_to_check))
            res = cr.fetchone()
            if res:
    ...
    def _check_record_rules_result_count(self, cr, uid, ids, result_ids, operation, context=None):
        ...
        if missing_ids:
            ...
            if forbidden_ids:
                ...
                if uid == SUPERUSER_ID:
                    return
                _logger.warning('Access Denied by record rules for operation: %s on record ids: %r, uid: %s, model: %s',
                                operation, forbidden_ids, uid, self._name)
                ...
                if operation in ('read', 'unlink'):
                    ...
                    return
                _logger.warning('Failed operation on deleted record(s): %s, uid: %s, model: %s', operation, uid,
                                self._name)
                raise MissingError(
                    _(
    ...
    def check_access_rule(self, cr, uid, ids, operation, context=None):
        ...
        if self.is_transient():
            ...
            where_clause, where_params, tables = self.pool.get('ir.rule').domain_get(cr, uid, self._name, operation,
                                                                                     context=context)
            if where_clause:
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        property_ids = ir_property.search(cr, uid, [('res_id', 'in', ['%s,%s' % (self._name, i) for i in ids])],
                                          context=context)
        ...
        for sub_ids in cr.split_for_in_conditions(ids):
            ...
            reference_ids = pool_model_data.search(cr, SUPERUSER_ID,
            ...
            if totranslate:
                ...
                for f in direct:
                    if self._columns[f].translate:
                        ...
                        self.pool['ir.translation']._set_ids(cr, user, self._name + ',' + f, 'model', context['lang'],
                                                             ids, translation_value, src_trans)
        ...
        if parents_changed:
            if self.pool._init:
                ...
                for id in parents_changed:
                    ...
                    cr.execute(
                        'SELECT parent_right, id FROM %s WHERE %s ORDER BY %s' % (self._table, clause, parent_order),
                        params)
                    parents = cr.fetchall()
                    ...
                    if pleft < position:
                        cr.execute('update ' + self._table + ' set parent_left=parent_left+%s where parent_left>=%s',
                                   (distance, position))
                        cr.execute('update ' + self._table + ' set parent_right=parent_right+%s where parent_right>=%s',
                                   (distance, position))
                        cr.execute(
                            'update ' + self._table + ' set parent_left=parent_left+%s, parent_right=parent_right+%s where parent_left>=%s and parent_left<%s',
                            (position - pleft, position - pleft, pleft, pright))
                    else:
                        cr.execute('update ' + self._table + ' set parent_left=parent_left+%s where parent_left>=%s',
                                   (distance, position))
                        cr.execute('update ' + self._table + ' set parent_right=parent_right+%s where parent_right>=%s',
                        ...
                        cr.execute(
                            'update ' + self._table + ' set parent_left=parent_left-%s, parent_right=parent_right-%s where parent_left>=%s and parent_left<%s',
                            (pleft - position + distance, pleft - position + distance, pleft + distance,
                             pright + distance))
    ...
    def _create(self, cr, user, vals, context=None):
        ...
        if self._parent_store and not context.get('defer_parent_store_computation'):
            if self.pool._init:
                ...
                if parent:
                    cr.execute(
                        'select parent_right from ' + self._table + ' where ' + self._parent_name + '=%s order by ' + (
                ...
                cr.execute('update ' + self._table + ' set parent_left=%s,parent_right=%s where id=%s',
                           (pleft + 1, pleft + 2, id_new))
                recs.invalidate_cache(['parent_left', 'parent_right'])
    ...
    def _store_get_values(self, cr, uid, ids, fields, context):
        ...
        for store_trigger in triggers_to_compute:
            ...
            if target_func_id_ not in target_id_results:
                ...
                target_id_results[target_func_id_] = [i for i in
                                                      store_trigger[target_ids_func_](self, cr, SUPERUSER_ID, ids,
                                                                                      context) if i]
    ...
    def _check_qorder(self, word):
        if not regex_order.match(word):
            raise except_orm(_('AccessError'), _(
    ...
    def _apply_ir_rules(self, cr, uid, query, mode='read', context=None):
        ...
        def apply_rule(added_clause, added_params, added_tables, parent_model=None):
            ...
            if added_clause:
                if parent_model:
                    ...
                    added_clause = [clause.replace('"%s"' % parent_table, '"%s"' % parent_alias) for clause in
                                    added_clause]
        ...
        rule_where_clause, rule_where_clause_params, rule_tables = rule_obj.domain_get(cr, uid, self._name, mode,
                                                                                       context=context)
        apply_rule(rule_where_clause, rule_where_clause_params, rule_tables)

        # apply ir.rules from the parents (through _inherits)
        for inherited_model in self._inherits:
            rule_where_clause, rule_where_clause_params, rule_tables = rule_obj.domain_get(cr, uid, inherited_model,
    ...
    def distinct_field_get(self, cr, uid, field, value, args=None, offset=0, limit=None):
        ...
        if field in self._inherit_fields:
            return self.pool[self._inherit_fields[field][0]].distinct_field_get(cr, uid, field, value, args, offset,
                                                                                limit)
        else:
