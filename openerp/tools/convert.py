...
form: module.record_id""" % (xml_id,)
            if module != self.module:
                modcnt = self.pool['ir.module.module'].search_count(self.cr, self.uid, ['&', ('name', '=', module),
                                                                                        ('state', 'in', ['installed'])])
    ...
    def _remove_ir_values(self, cr, name, value, model):
        ...
        ir_value_ids = ir_values_obj.search(cr, self.uid,
    ...
    def _tag_report(self, cr, rec, data_node=None, mode=None):
        ...
        id = self.pool['ir.model.data']._update(cr, self.uid, "ir.actions.report.xml", self.module, res, xml_id,
        ...
        if not rec.get('menu') or eval(rec.get('menu', 'False')):
            ...
            self.pool['ir.model.data'].ir_set(cr, self.uid, 'action', keyword, res['name'], [res['model']], value,
    ...
    def _tag_url(self, cr, rec, data_node=None, mode=None):
        ...
        id = self.pool['ir.model.data']._update(cr, self.uid, "ir.actions.act_url", self.module, res, xml_id,
                                                noupdate=self.isnoupdate(data_node), mode=self.mode)
        self.idref[xml_id] = int(id)
    ...
    def _tag_act_window(self, cr, rec, data_node=None, mode=None):
        ...
        id = self.pool['ir.model.data']._update(cr, self.uid, 'ir.actions.act_window', self.module, res, xml_id,
                                                noupdate=self.isnoupdate(data_node), mode=self.mode)
        self.idref[xml_id] = int(id)
        ...
        if src_model:
            ...
            self.pool['ir.model.data'].ir_set(cr, self.uid, 'action', keyword, xml_id, [src_model], value,
                                              replace=replace, isobject=True, xml_id=xml_id)
    ...
    def _tag_ir_set(self, cr, rec, data_node=None, mode=None):
        ...
        for field in rec.findall('./field'):
            ...
            res[f_name] = f_val
        self.pool['ir.model.data'].ir_set(cr, self.uid, res['key'], res['key2'], res['name'], res['models'],
                                          res['value'], replace=res.get('replace', True),
    ...
    def _tag_record(self, cr, rec, data_node=None, mode=None):
        ...
        id = self.pool['ir.model.data']._update(cr, self.uid, rec_model, self.module, res, rec_id or False,
                                                not self.isnoupdate(data_node), noupdate=self.isnoupdate(data_node),
            ...
            for rec in n:
                if rec.tag in self._tags:
                    try:
                        ...
                        raise ParseError, (
                        misc.ustr(e), etree.tostring(rec).rstrip(), rec.getroottree().docinfo.URL, rec.sourceline), \
                        exc_info[2]
        return True
...
def convert_csv_import(cr, module, fname, csvcontent, idref=None, mode='init',
    ...
    result, rows, warning_msg, dummy = registry[model].import_data(cr, uid, fields, datas, mode, module, noupdate,
                                                                   filename=fname_partial)
    if result < 0:
