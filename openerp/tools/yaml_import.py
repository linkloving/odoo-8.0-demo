...
class YamlInterpreter(object):
            ...
            for id in ids:
                ...
                for test in expressions:
                    try:
                        ...
                        _logger.debug('Exception during evaluation of !assert block in yaml_file %s.', self.filename,
                                      exc_info=True)
    ...
    def process_record(self, node):
        ...
        if model.is_transient():
            ...
            if self.isnoupdate(record) and self.mode != 'init':
                id = self.pool['ir.model.data']._update_dummy(self.cr, SUPERUSER_ID, record.model, self.module,
                                                              record.id)
    ...
    def _create_record(self, model, fields, view_info=None, parent={}, default=True):
        ...
        def _get_right_one2many_view(fg, field_name, view_type):
            ...
            if not one2many_view:
                one2many_view = self.pool[fg[field_name]['relation']].fields_view_get(self.cr, SUPERUSER_ID, False,
                                                                                      view_type, self.context)
            ...
            while nodes:
                ...
                if el.tag == 'field':
                    ...
                    if field_name in fields:
                        ...
                        field_value = self._eval_field(model, field_name, fields[field_name],
                                                       one2many_form_view or view_info, parent=record_dict,
                                                       default=default)
    ...
    def _eval_field(self, model, field_name, expression, view_info=False, parent={}, default=True):
        ...
        if is_ref(expression):
            ...
            value = [(0, 0, self._create_record(other_model, fields, view_info, parent, default=default)) for fields in
                     expression]
    ...
    def process_menuitem(self, node):
        ...
        if node.action:
            ...
            if action_type == 'act_window':
                ...
                self.cr.execute('select view_type,view_mode,name,view_id,target from ir_act_window where id=%s',
                                (action_id,))
                ...
                self.cr.execute(
                    'SELECT view_mode FROM ir_act_window_view WHERE act_window_id=%s ORDER BY sequence LIMIT 1',
                    (action_id,))
                if self.cr.rowcount:
                    action_mode = self.cr.fetchone()
                if action_type == 'tree':
        ...
        if node.action and pid:
            ...
            self.pool['ir.model.data'].ir_set(self.cr, SUPERUSER_ID, 'action', \
                                              'tree_but_open', 'Menuitem', [('ir.ui.menu', int(pid))], action, True,
                                              True, xml_id=node.id)
    ...
    def process_act_window(self, node):
        ...
        if node.src_model:
            ...
            self.pool['ir.model.data'].ir_set(self.cr, SUPERUSER_ID, 'action', keyword, \
                                              node.id, [node.src_model], value, replace=replace,
    ...
    def process_report(self, node):
        ...
        for field, dest in (('rml', 'report_rml'), ('file', 'report_rml'), ('xml', 'report_xml'), ('xsl', 'report_xsl'),
                            ('attachment', 'attachment'), ('attachment_use', 'attachment_use')):
        ...
        id = self.pool['ir.model.data']._update(self.cr, SUPERUSER_ID, "ir.actions.report.xml", \
                                                self.module, values, xml_id, noupdate=self.isnoupdate(node),
                                                mode=self.mode)
        ...
        if not node.menu or eval(node.menu):
            ...
            replace = node.replace or True
            self.pool['ir.model.data'].ir_set(self.cr, SUPERUSER_ID, 'action', \
                                              keyword, values['name'], [values['model']], value, replace=replace,
...
def yaml_import(cr, module, yamlfile, kind, idref=None, mode='init', noupdate=False, report=None):
    ...
    yaml_interpreter = YamlInterpreter(cr, module, idref, mode, filename=yamlfile.name, report=report,
                                       noupdate=noupdate, loglevel=loglevel)
