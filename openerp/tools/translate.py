...
def translate(cr, name, source_type, lang, source=None):
    if source and name:
        cr.execute('select value from ir_translation where lang=%s and type=%s and name=%s and src=%s',
        ...
        try:
            ...
            if lang:
                ...
                if cr:
                    ...
                    res = registry['ir.translation']._get_source(cr, SUPERUSER_ID, None, ('code', 'sql_constraint'),
                                                                 lang, source)
                else:
...
def trans_load(cr, filename, lang, verbose=True, module_name=None, context=None):
    try:
        ...
        result = trans_load_data(cr, fileobj, fileformat, lang, verbose=verbose, module_name=module_name,
                                 context=context)
        fileobj.close()
        return result
    except IOError:
