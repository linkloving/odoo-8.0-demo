...
class res_currency(osv.osv):
        ...
        for id in ids:
            ...
            if cr.rowcount:
                ...
                raise osv.except_osv(_('Error!'), _(
                    "No currency rate associated for currency '%s' for the given period" % (currency.name)))
        return res
    ...
    def get_format_currencies_js_function(self, cr, uid, context=None):
        ...
        function = ""
        for row in self.search_read(cr, uid, domain=[], fields=['id', 'name', 'symbol', 'rounding', 'position'],
                                    context=context):
            digits = int(math.ceil(math.log10(1 / row['rounding'])))
            symbol = row['symbol'] or row['name']

            format_number_str = "openerp.web.format_value(arguments[0], {type: 'float', digits: [69," + str(
                digits) + "]}, 0.00)"
    ...
    def name_search(self, cr, user, name, args=None, operator='ilike', context=None, limit=80):
        if operator in ['=', '!=']:
            try:
                ...
                name = ''
                operator = 'ilike'
        return super(res_currency_rate, self).name_search(cr, user, name, args=args, operator=operator, context=context,
                                                          limit=limit)
