...
class wkf_activity(osv.osv):
    ...
    _columns = {
        ...
        'split_mode': fields.selection([('XOR', 'Xor'), ('OR', 'Or'), ('AND', 'And')], 'Split Mode', size=3,
                                       required=True),
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        if not context.get('_force_unlink') and self.pool.get('workflow.workitem').search(cr, uid,
                                                                                          [('act_id', 'in', ids)]):
...
class wkf_transition(osv.osv):
    ...
    _columns = {
        ...
        'wkf_id': fields.related('act_from', 'wkf_id', type='many2one', relation='workflow', string='Workflow',
                                 select=True),
    }
    _defaults = {
    ...
    def name_get(self, cr, uid, ids, context=None):
        return [(line.id, (line.act_from.name) + '+' + (line.act_to.name)) if line.signal == False else (
        line.id, line.signal) for line in self.browse(cr, uid, ids, context=context)]

    def name_search(self, cr, user, name, args=None, operator='ilike', context=None, limit=100):
        if args is None:
            args = []
        if name:
            ids = self.search(cr, user, ['|', ('act_from', operator, name), ('act_to', operator, name)] + args,
                              limit=limit)
            return self.name_get(cr, user, ids, context=context)
        return super(wkf_transition, self).name_search(cr, user, name, args=args, operator=operator, context=context,
                                                       limit=limit)
