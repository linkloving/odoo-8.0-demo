...
class TestServerActionsBase(common.TransactionCase):
    def setUp(self):
        ...
        self.res_partner_name_field_id = \
        self.ir_model_fields.search(cr, uid, [('model', '=', 'res.partner'), ('name', '=', 'name')])[0]
        self.res_partner_city_field_id = \
        self.ir_model_fields.search(cr, uid, [('model', '=', 'res.partner'), ('name', '=', 'city')])[0]
        self.res_partner_country_field_id = \
        self.ir_model_fields.search(cr, uid, [('model', '=', 'res.partner'), ('name', '=', 'country_id')])[0]
        self.res_partner_parent_field_id = \
        self.ir_model_fields.search(cr, uid, [('model', '=', 'res.partner'), ('name', '=', 'parent_id')])[0]
        self.res_country_model_id = self.ir_model.search(cr, uid, [('model', '=', 'res.country')])[0]
        self.res_country_name_field_id = \
        self.ir_model_fields.search(cr, uid, [('model', '=', 'res.country'), ('name', '=', 'name')])[0]
        self.res_country_code_field_id = \
        self.ir_model_fields.search(cr, uid, [('model', '=', 'res.country'), ('name', '=', 'code')])[0]
...
class TestServerActions(TestServerActionsBase):
    def test_00_action(self):
        ...
        self.assertEqual(len(ir_values_ids), 1,
                         'ir_actions_server: create_action should have created an entry in ir_values')
        ir_value = self.ir_values.browse(cr, uid, ir_values_ids[0])
        self.assertEqual(ir_value.value, 'ir.actions.server,%s' % self.act_id,
                         'ir_actions_server: created ir_values should reference the server action')
        self.assertEqual(ir_value.model, 'res.partner',
...
workflow"""
    ...
    def test_40_crud_create(self):
        ...
        self.assertFalse(run_res, 'ir_actions_server: create record action correctly finished should return False')
        # Test: new partner created
        pids = self.res_partner.search(cr, uid, [('name', 'ilike',
                                                  'TestingPartner (copy)')])  # currently res_partner overrides default['name'] whatever its value
        self.assertEqual(len(pids), 1, 'ir_actions_server: TODO')
        partner = self.res_partner.browse(cr, uid, pids[0])
        ...
        self.ir_actions_server.write(cr, uid, [self.act_id], {
            ...
            'fields_lines': [(0, 0, {'col1': self.res_country_name_field_id, 'value': 'obj.name', 'type': 'equation'}),
                             (0, 0,
                              {'col1': self.res_country_code_field_id, 'value': 'obj.name[0:2]', 'type': 'equation'})],
        })
        run_res = self.ir_actions_server.run(cr, uid, [self.act_id], context=self.context)
        self.assertFalse(run_res, 'ir_actions_server: create record action correctly finished should return False')
