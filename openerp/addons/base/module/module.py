...
class module(osv.osv):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        res = super(module, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context,
                                                  toolbar=toolbar, submenu=False)
        result = \
        self.pool.get('ir.model.data').get_object_reference(cr, uid, 'base', 'action_server_module_immediate_install')[
            1]
        if view_type == 'form':
    ...
    _columns = {
        ...
        'description_html': fields.function(_get_desc, string='Description HTML', type='html', method=True,
                                            readonly=True),
    ...
    def install_from_urls(self, cr, uid, urls, context=None):
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    ...
                    raise osv.except_osv(_('Module not found'),
                                         _(
                                             'The `%s` module appears to be unavailable at the moment, please try again later.') % module_name)
                else:
            ...
            already_installed = self.search(cr, uid, [('id', 'in', downloaded_ids), ('state', '=', 'installed')],
                                            context=context)

            to_install_ids = self.search(cr, uid, [('name', 'in', urls.keys()), ('state', '=', 'uninstalled')],
                                         context=context)
    ...
    def _update_dependencies(self, cr, uid, mod_browse, depends=None):
        ...
        for dep in (needed - existing):
            cr.execute('INSERT INTO ir_module_module_dependency (module_id, name) values (%s, %s)',
                       (mod_browse.id, dep))
        for dep in (existing - needed):
            cr.execute('DELETE FROM ir_module_module_dependency WHERE module_id = %s and name = %s',
                       (mod_browse.id, dep))
