...
class TestRelatedField(common.TransactionCase):
    ...
    def test_3_read_write(self):
        ...
        partner_ids = self.partner.search(self.cr, self.uid,
                                          [('related_company_partner_id', '=', company.partner_id.id)])
...
class TestPropertyField(common.TransactionCase):
    ...
    def test_1_property_multicompany(self):
        ...
        self.assertEqual(self.partner.browse(cr, bob, partner_id).property_country.id, country_fr,
        ...
        self.assertEqual(self.partner.browse(cr, alice, partner_id).property_country.id, country_be,
                         "Alice does not see the value he has set on the property field")
        self.assertEqual(self.partner.browse(cr, bob, partner_id).property_country.id, country_fr,
                         "Changes made by Alice have overwritten Bob's value")
...
%endif"""
        ...
        self.assertEqual(partner.comment, some_ugly_html,
