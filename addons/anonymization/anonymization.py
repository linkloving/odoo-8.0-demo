...
ANONYMIZATION_DIRECTION = [('clear -> anonymized', 'clear -> anonymized'),
...
class ir_model_fields_anonymization(osv.osv):
    ...
    _sql_constraints = [
        ('model_id_field_id_uniq', 'unique (model_name, field_name)',
         _("You cannot have two fields with the same name on the same object!")),
    ]
        ...
        if context.get('manual'):
            ...
            if global_state == 'anonymized':
                raise osv.except_osv('Error!',
                                     "The database is currently anonymized, you cannot create, modify or delete fields.")
    ...
    def _get_model_and_field_ids(self, cr, uid, vals, context=None):
        ...
        if 'field_name' in vals and vals['field_name'] and 'model_name' in vals and vals['model_name']:
            ...
            if model_ids:
                field_ids = ir_model_fields_obj.search(cr, uid, [('name', '=', vals['field_name']),
                                                                 ('model_id', '=', model_ids[0])], context=context)
                if field_ids:
...
class ir_model_fields_anonymization_history(osv.osv):
    ...
    _columns = {
        ...
        'field_ids': fields.many2many('ir.model.fields.anonymization', 'anonymized_field_to_history_rel', 'field_id',
                                      'history_id', 'Fields', readonly=True),
        'state': fields.selection(selection=ANONYMIZATION_HISTORY_STATE, string='Status', required=True, readonly=True),
        'direction': fields.selection(selection=ANONYMIZATION_DIRECTION, string='Direction', size=20, required=True,
                                      readonly=True),
...
class ir_model_fields_anonymize_wizard(osv.osv_memory):
    ...
    _columns = {
        ...
        'state': fields.function(_get_state, string='Status', type='selection', selection=WIZARD_ANONYMIZATION_STATES,
                                 readonly=False),
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, *args, **kwargs):
        ...
        res = super(ir_model_fields_anonymize_wizard, self).fields_view_get(cr, uid, view_id, view_type,
    ...
    def anonymize_database(self, cr, uid, ids, context=None):
        ...
        state = self.pool.get('ir.model.fields.anonymization')._get_global_state(cr, uid, context=context)
        if state == 'anonymized':
            self._raise_after_history_update(cr, uid, history_id, _('Error !'),
        ...
        field_ids = ir_model_fields_anonymization_model.search(cr, uid, [('state', '<>', 'not_existing')],
                                                               context=context)
        ...
        for field in fields:
            ...
            for record in records:
                ...
                if field_type == 'char':
                    ...
                    anonymized_value = 0
                elif field_type in ['binary', 'many2many', 'many2one', 'one2many',
                                    'reference']:  # cannot anonymize these kind of fields
                    msg = _("Cannot anonymize fields of these types: binary, many2many, many2one, one2many, reference.")
                    self._raise_after_history_update(cr, uid, history_id, 'Error !', msg)

                if anonymized_value is None:
                    self._raise_after_history_update(cr, uid, history_id, _('Error !'),
                                                     _("Anonymized value is None. This cannot happens."))
    ...
    def reverse_anonymize_database(self, cr, uid, ids, context=None):
        ...
        if state == 'clear':
            raise osv.except_osv_('Error!',
                                  "The database is not currently anonymized, you cannot reverse the anonymization.")
        elif state == 'unstable':
            msg = _("The database anonymization is currently in an unstable state. Some fields are anonymized," + \
                    " while some fields are not anonymized. You should try to solve this problem before trying to do anything.")
            raise osv.except_osv('Error!', msg)
        ...
        for wizard in wizards:
            ...
            for line in data:
                ...
                if custom_updates:
                    ...
                    queries = [(record['query'], record['query_type']) for record in custom_updates if
                               record['query_type']]
                elif table_name:
                ...
                for query in queries:
                    if query[1] == 'sql':
                        ...
                        raise Exception(
                            "Unknown query type '%s'. Valid types are: sql, python." % (query['query_type'],))
