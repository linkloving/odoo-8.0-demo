...
class product_product(osv.osv):
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False,
        ...
        """
        res = super(product_product, self).read_group(cr, uid, domain, fields, groupby, offset=offset, limit=limit,
        ...
        if any(x in fields for x in fields_list):
            ...
            res_val = self._product_margin(cr, uid, tot_products, [x for x in fields if fields in fields_list], '',
                                           context=context)
    ...
    def _product_margin(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for val in self.browse(cr, uid, ids, context=context):
            ...
            if 'total_margin_rate' in field_names:
                res[val.id]['total_margin_rate'] = res[val.id]['turnover'] and res[val.id]['total_margin'] * 100 / \
                                                                               res[val.id]['turnover'] or 0.0
            if 'expected_margin_rate' in field_names:
                res[val.id]['expected_margin_rate'] = res[val.id]['sale_expected'] and res[val.id][
                                                                                           'expected_margin'] * 100 / \
                                                                                       res[val.id][
                                                                                           'sale_expected'] or 0.0
        return res
    ...
    _columns = {
        ...
        'sale_avg_price': fields.function(_product_margin, type='float', string='Avg. Unit Price',
                                          multi='product_margin',
                                          help="Avg. Price in Customer Invoices."),
        'purchase_avg_price': fields.function(_product_margin, type='float', string='Avg. Unit Price',
                                              multi='product_margin',
                                              help="Avg. Price in Supplier Invoices "),
        'sale_num_invoiced': fields.function(_product_margin, type='float', string='# Invoiced in Sale',
                                             multi='product_margin',
                                             help="Sum of Quantity in Customer Invoices"),
        'purchase_num_invoiced': fields.function(_product_margin, type='float', string='# Invoiced in Purchase',
                                                 multi='product_margin',
        ...
        'expected_margin': fields.function(_product_margin, type='float', string='Expected Margin',
                                           multi='product_margin',
                                           help="Expected Sale - Normal Cost"),
        'total_margin_rate': fields.function(_product_margin, type='float', string='Total Margin Rate(%)',
                                             multi='product_margin',
                                             help="Total margin * 100 / Turnover"),
        'expected_margin_rate': fields.function(_product_margin, type='float', string='Expected Margin (%)',
                                                multi='product_margin',
