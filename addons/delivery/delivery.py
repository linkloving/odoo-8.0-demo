...
class delivery_carrier(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        if not order_id:
            ...
            res = [(r['id'], r['name'] + ' (' + (str(r['price'])) + ' ' + currency + ')') for r in
                   self.read(cr, uid, ids, ['name', 'price'], context)]
        return res
    ...
    _columns = {
        ...
        'free_if_more_than': fields.boolean('Free If Order Total Amount Is More Than',
...
class delivery_grid(osv.osv):
    ...
    _columns = {
        ...
        'country_ids': fields.many2many('res.country', 'delivery_grid_country_rel', 'grid_id', 'country_id',
                                        'Countries'),
        ...
        'active': fields.boolean('Active',
                                 help="If the active field is set to False, it will allow you to hide the delivery grid without removing it."),
    }
    _defaults = {
        'active': lambda *a: 1,
        'sequence': lambda *a: 1,
