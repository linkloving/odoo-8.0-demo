...
class product_product(osv.osv):
    ...
    def view_header_get(self, cr, user, view_id, view_type, context=None):
        ...
        if (context.get('active_id', False)) and (context.get('active_model') == 'stock.location'):
            return _('Products: ') + self.pool.get('stock.location').browse(cr, user, context['active_id'],
                                                                            context).name
        return res
    ...
    def _get_domain_locations(self, cr, uid, ids, context=None):
        ...
        if operator == "child_of" and locations and locations[0].parent_left != 0:
            ...
            return (
                ...
                domain + ['&', ('location_dest_id', operator, location_ids), '!',
    ...
    def _product_available(self, cr, uid, ids, field_names=None, arg=False, context=None):
        ...
        quants = self.pool.get('stock.quant').read_group(cr, uid, domain_quant, ['product_id', 'qty'], ['product_id'],
                                                         context=context)
    ...
    def _search_product_quantity(self, cr, uid, obj, name, domain, context):
        ...
        for field, operator, value in domain:
            ...
            assert field in (
            'qty_available', 'virtual_available', 'incoming_qty', 'outgoing_qty'), 'Invalid domain left operand'
...
class product_template(osv.osv):
    ...
    _columns = {
        ...
        'track_incoming': fields.boolean('Track Incoming Lots',
                                         help="Forces to specify a Serial Number for all moves containing this product and coming from a Supplier Location"),
        'track_outgoing': fields.boolean('Track Outgoing Lots',
                                         help="Forces to specify a Serial Number for all moves containing this product and going to a Customer Location"),
        'track_all': fields.boolean('Full Lots Traceability',
                                    help="Forces to specify a Serial Number on each and every operation related to this product"),
    ...
    def action_view_routes(self, cr, uid, ids, context=None):
        ...
        route_ids = route_obj.search(cr, uid,
                                     ['|', ('id', 'in', list(product_route_ids)), ('warehouse_selectable', '=', True)],
                                     context=context)
    ...
    def action_view_orderpoints(self, cr, uid, ids, context=None):
        ...
        if len(ids) == 1 and len(products) == 1:
            result['context'] = "{'default_product_id': " + str(products[0]) + ", 'search_default_product_id': " + str(
                products[0]) + "}"
        else:
            result['domain'] = "[('product_id','in',[" + ','.join(map(str, products)) + "])]"
            result['context'] = "{}"
            ...
            for product in self.browse(cr, uid, ids, context=context):
                ...
                if old_uom != new_uom:
                    if self.pool.get('stock.move').search(cr, uid, [
                        ('product_id', 'in', [x.id for x in product.product_variant_ids]), ('state', '=', 'done')],
...
class product_putaway_strategy(osv.osv):
    ...
    _columns = {
        ...
        'fixed_location_ids': fields.one2many('stock.fixed.putaway.strat', 'putaway_id',
...
class product_category(osv.osv):
    ...
    _columns = {
        'route_ids': fields.many2many('stock.location.route', 'stock_location_route_categ', 'categ_id', 'route_id',
                                      'Routes', domain="[('product_categ_selectable', '=', True)]"),
        ...
        'total_route_ids': fields.function(calculate_total_routes, relation='stock.location.route', type='many2many',
                                           string='Total routes', readonly=True),
    }
