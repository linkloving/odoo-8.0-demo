...
class stock_graph(object):
    ...
    def draw(self):
        ...
        x_axis = axis.X(format=lambda x: '/a60{}' + time.strftime('%Y-%m-%d', time.gmtime(x)), tic_interval=interval,
                        label=None)
        ...
        tb.draw()
        ar = area.T(size=(620, 435), x_range=(val_min, val_max + 1),
                    y_axis=axis.Y(format="%d", label="Virtual Stock (Unit)"), x_axis=x_axis)
        for plot in plots:
            ar.add_plot(plot)
