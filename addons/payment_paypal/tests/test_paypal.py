...
class PaypalCommon(PaymentAcquirerCommon):
    def setUp(self):
        ...
        model, self.paypal_id = self.registry('ir.model.data').get_object_reference(cr, uid, 'payment_paypal',
...
class PaypalForm(PaypalCommon):
    def test_10_paypal_form_render(self):
        ...
        self.assertEqual(tree.get('action'), 'https://www.sandbox.paypal.com/cgi-bin/webscr',
    ...
    def test_20_paypal_form_management(self):
        ...
        self.assertEqual(tx.paypal_txn_id, '08D73520KX778924N',
                         'paypal: wrong txn_id after receiving a valid pending notification')
        self.assertFalse(tx.date_validate,
                         'paypal: validation date should not be updated whenr receiving pending notification')
