                ...
                if self.OE.update.split('.')[0] != self.GG.update.split('.')[0]:
                    ...
                    if self[tmpSrc].isRecurrence:
                        if self[tmpSrc].status:
                            ...
                            self.OP = Exclude(tmpSrc,
                                              'Need to Exclude (Me = First event from recurrence) from recurrence')
                    ...
                    if not self.OE.synchro or self.OE.synchro.split('.')[0] < self.OE.update.split('.')[0]:
                        self.OP = Update('OE',
                                         'Event already updated by another user, but not synchro with my google calendar')
                    else:
...
class google_calendar(osv.AbstractModel):
    ...
    def generate_data(self, cr, uid, event, isCreating=False, context=None):
        if not context:
            context = {}
        if event.allday:
            start_date = fields.datetime.context_timestamp(cr, uid, datetime.strptime(event.start,
                                                                                      tools.DEFAULT_SERVER_DATETIME_FORMAT),
                                                           context=context).isoformat('T').split('T')[0]
            final_date = fields.datetime.context_timestamp(cr, uid, datetime.strptime(event.stop,
                                                                                      tools.DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(
                days=1), context=context).isoformat('T').split('T')[0]
            type = 'date'
            vstype = 'dateTime'
        else:
            start_date = fields.datetime.context_timestamp(cr, uid, datetime.strptime(event.start,
                                                                                      tools.DEFAULT_SERVER_DATETIME_FORMAT),
                                                           context=context).isoformat('T')
            final_date = fields.datetime.context_timestamp(cr, uid, datetime.strptime(event.stop,
                                                                                      tools.DEFAULT_SERVER_DATETIME_FORMAT),
                                                           context=context).isoformat('T')
            type = 'dateTime'
        ...
        if not self.get_need_synchro_attendee(cr, uid, context=context):
            data.pop("attendees")
        if isCreating:
            other_google_ids = [other_att.google_internal_event_id for other_att in event.attendee_ids if
    ...
    def create_an_event(self, cr, uid, event, context=None):
        ...
        'primary', urllib2.quote('id,updated'), self.get_token(cr, uid, context))
    ...
    def get_calendar_primary_id(self, cr, uid, context=None):
        ...
        try:
            st, content, ask_time = self.pool['google.service']._do_request(cr, uid, url, params, headers, type='GET',
                                                                            context=context)
        except Exception, e:
            ...
            if (e.code == 401):  # Token invalid / Acces unauthorized
                ...
                with registry.cursor() as cur:
                    self.pool['res.users'].write(cur, SUPERUSER_ID, [uid], {'google_calendar_token': False,
    ...
    def get_event_synchro_dict(self, cr, uid, lastSync=False, token=False, nextPageToken=False, context=None):
        ...
        status, content, ask_time = self.pool['google.service']._do_request(cr, uid, url, params, headers, type='GET',
                                                                            context=context)
        ...
        if content.get('nextPageToken'):
            google_events_dict.update(
                self.get_event_synchro_dict(cr, uid, lastSync=lastSync, token=token,
                                            nextPageToken=content['nextPageToken'], context=context)
            )
    ...
    def get_one_event_synchro(self, cr, uid, google_id, context=None):
        ...
        try:
            status, content, ask_time = self.pool['google.service']._do_request(cr, uid, url, params, headers,
    ...
    def update_to_google(self, cr, uid, oe_event, google_event, context):
        ...
        'primary', google_event['id'], 'id,updated', self.get_token(cr, uid, context))
        headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
        data = self.generate_data(cr, uid, oe_event, context=context)
        data['sequence'] = google_event.get('sequence', 0)
        data_json = simplejson.dumps(data)

        status, content, ask_time = self.pool['google.service']._do_request(cr, uid, url, data_json, headers,
                                                                            type='PATCH', context=context)
        ...
        if context['curr_attendee']:
            self.pool['calendar.attendee'].write(cr, uid, [context['curr_attendee']], {'oe_synchro_date': update_date},
                                                 context)
    ...
    def update_an_event(self, cr, uid, event, context=None):
        ...
        status, response, ask_time = self.pool['google.service']._do_request(cr, uid, url, data, headers, type='GET',
                                                                             context=context)
    ...
    def update_recurrent_event_exclu(self, cr, uid, instance_id, event_ori_google_id, event_new, context=None):
        ...
        url = "/calendar/v3/calendars/%s/events/%s?access_token=%s" % (
        'primary', instance_id, self.get_token(cr, uid, context))
    ...
    def create_from_google(self, cr, uid, event, partner_id, context=None):
        ...
        attendee_record_id = self.pool['calendar.attendee'].search(cr, uid, [('partner_id', '=', partner_id),
                                                                             ('event_id', '=', res)], context=context)
        self.pool['calendar.attendee'].write(cr, uid, attendee_record_id, {'oe_synchro_date': meeting.oe_update_date,
                                                                           'google_internal_event_id': event.GG.event[
                                                                               'id']}, context=context_tmp)
        if meeting.recurrency:
            attendee_ids = self.pool['calendar.attendee'].search(cr, SUPERUSER_ID, [
                ('google_internal_event_id', '=ilike', '%s\_%%' % event.GG.event['id'])], context=context)
    ...
    def update_from_google(self, cr, uid, event, single_event_dict, type, context):
                ...
                if type == "write":
                    for oe_attendee in event['attendee_ids']:
                        if oe_attendee.email == partner_email:
                            calendar_attendee_obj.write(cr, uid, [oe_attendee.id],
                                                        {'state': google_attendee['responseStatus']}, context=context)
        ...
        if context['curr_attendee']:
            self.pool['calendar.attendee'].write(cr, uid, [context['curr_attendee']], {'oe_synchro_date': update_date,
                                                                                       'google_internal_event_id': single_event_dict.get(
                                                                                           'id', False)}, context)
        return res
    ...
    def remove_references(self, cr, uid, context=None):
        ...
        all_my_attendees = self.pool['calendar.attendee'].search(cr, uid,
                                                                 [('partner_id', '=', current_user.partner_id.id)],
                                                                 context=context)
        self.pool['calendar.attendee'].write(cr, uid, all_my_attendees,
                                             {'oe_synchro_date': False, 'google_internal_event_id': False},
                                             context=context)
        ...
        for user_to_sync in ids:
            ...
            try:
                ...
                if resp.get("status") == "need_reset":
                    ...
                    _logger.info(
                        "[%s] Calendar Synchro - Done with status : %s  !" % (user_to_sync, resp.get("status")))
            except Exception, e:
    ...
    def synchronize_events(self, cr, uid, ids, lastSync=True, context=None):
        ...
        if current_user.google_calendar_cal_id:
            ...
            if lastSync and self.get_last_sync_date(cr, user_to_sync,
                                                    context=context) and not self.get_disable_since_synchro(cr,
                                                                                                            user_to_sync,
                                                                                                            context=context):
                lastSync = self.get_last_sync_date(cr, user_to_sync, context)
                _logger.info("[%s] Calendar Synchro - MODE SINCE_MODIFIED : %s !" % (
                user_to_sync, lastSync.strftime(DEFAULT_SERVER_DATETIME_FORMAT)))
            else:
                lastSync = False
    ...
    def create_new_events(self, cr, uid, context=None):
        ...
        my_att_ids = att_obj.search(cr, uid, [('partner_id', '=', myPartnerID),
                                              ...
                                              '|',
                                              ('event_id.stop', '>',
                                               self.get_minTime(cr, uid, context=context).strftime(
        ...
        for att in att_obj.browse(cr, uid, my_att_ids, context=context):
            other_google_ids = [other_att.google_internal_event_id for other_att in att.event_id.attendee_ids if
                                other_att.google_internal_event_id and other_att.id != att.id]
                ...
                if not att.event_id.recurrent_id or att.event_id.recurrent_id == 0:
                    ...
                    if status_response(st):
                        ...
                        att_obj.write(cr, uid, [att.id],
                                      {'google_internal_event_id': response['id'], 'oe_synchro_date': update_date})
                        cr.commit()
                    else:
    ...
    def bind_recurring_events_to_google(self, cr, uid, context=None):
        ...
        my_att_ids = att_obj.search(cr, uid,
                                    [('partner_id', '=', myPartnerID), ('google_internal_event_id', '=', False)],
                                    context=context_norecurrent)
        ...
        for att in att_obj.browse(cr, uid, my_att_ids, context=context):
            if att.event_id.recurrent_id and att.event_id.recurrent_id > 0:
                ...
                source_attendee_record_id = att_obj.search(cr, uid, [('partner_id', '=', myPartnerID),
                                                                     ('event_id', '=', source_event_record.id)],
                                                           context=context)
                ...
                if att.event_id.recurrent_id_date and source_event_record.allday and source_attendee_record.google_internal_event_id:
                    new_google_internal_event_id = source_attendee_record.google_internal_event_id + '_' + \
                    ...
                    new_google_internal_event_id = source_attendee_record.google_internal_event_id + '_' + att.event_id.recurrent_id_date.replace(
                        '-', '').replace(' ', 'T').replace(':', '') + 'Z'

                if new_google_internal_event_id:
                    # TODO WARNING, NEED TO CHECK THAT EVENT and ALL instance NOT DELETE IN GMAIL BEFORE !
                    try:
                        st, response, ask_time = self.update_recurrent_event_exclu(cr, uid,
                                                                                   new_google_internal_event_id,
                                                                                   source_attendee_record.google_internal_event_id,
                                                                                   att.event_id, context=context)
                        if status_response(st):
    ...
    def update_events(self, cr, uid, lastSync=False, context=None):
        ...
        if lastSync:
            try:
                ...
                if e.code == 410:  # GONE, Google is lost.
                    ...
                    registry = openerp.modules.registry.RegistryManager.get(request.session.db)
                    with registry.cursor() as cur:
                        self.pool['res.users'].write(cur, SUPERUSER_ID, [uid],
                                                     {'google_calendar_last_sync_date': False}, context=context)
            ...
            my_openerp_att_ids = att_obj.search(cr, uid, [
                ...
                ('event_id.oe_update_date', '>',
                 lastSync and lastSync.strftime(DEFAULT_SERVER_DATETIME_FORMAT) or self.get_minTime(cr, uid,
                                                                                                    context).strftime(
                     DEFAULT_SERVER_DATETIME_FORMAT)),
                ('google_internal_event_id', '!=', False),
            ], context=context_novirtual)

            my_openerp_googleinternal_ids = att_obj.read(cr, uid, my_openerp_att_ids,
            ...
            for giid in my_openerp_googleinternal_ids:
                ...
                if giid.get('event_id'):
                    active = calendar_event.browse(cr, uid, int(giid.get('event_id')[0]),
                                                   context=context_novirtual).active

                if giid.get('google_internal_event_id') and not all_event_from_google.get(
                        giid.get('google_internal_event_id')) and active:
                    one_event = self.get_one_event_synchro(cr, uid, giid.get('google_internal_event_id'),
                                                           context=context)
                    if one_event:
            ...
            domain = [
                ...
                ('event_id.final_date', '>',
                 self.get_minTime(cr, uid, context).strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
            ]
        ...
        ######################
        for base_event in event_to_synchronize:
            event_to_synchronize[base_event] = sorted(event_to_synchronize[base_event].iteritems(),
                    ...
                    if actSrc == 'OE':
                        ...
                        if event.GG.status:
                            ...
                            if not event_to_synchronize[base_event][0][1].OE.event_id:
                                main_ev = att_obj.search_read(cr, uid, [
                                    ('google_internal_event_id', '=', event.GG.event['id'].rsplit('_', 1)[0])],
                                                              fields=['event_id'], context=context_novirtual)
                                event_to_synchronize[base_event][0][1].OE.event_id = main_ev[0].get('event_id')[
                                    0] if main_ev else None
                            ...
                            if parent_oe_id:
                                calendar_event.unlink(cr, uid, "%s-%s" % (parent_oe_id, new_google_event_id),
    ...
    def get_token(self, cr, uid, context=None):
        ...
        if not current_user.google_calendar_token_validity or \
                        datetime.strptime(current_user.google_calendar_token_validity.split('.')[0],
                                          DEFAULT_SERVER_DATETIME_FORMAT) < (datetime.now() + timedelta(minutes=1)):
    ...
    def get_last_sync_date(self, cr, uid, context=None):
        current_user = self.pool['res.users'].browse(cr, uid, uid, context=context)
        return current_user.google_calendar_last_sync_date and datetime.strptime(
            current_user.google_calendar_last_sync_date, DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(minutes=0) or False

    def do_refresh_token(self, cr, uid, context=None):
        current_user = self.pool['res.users'].browse(cr, uid, uid, context=context)
        gs_pool = self.pool['google.service']

        all_token = gs_pool._refresh_google_token_json(cr, uid, current_user.google_calendar_rtoken, self.STR_SERVICE,
                                                       context=context)
    ...
    def authorize_google_uri(self, cr, uid, from_url='http://www.openerp.com', context=None):
        url = self.pool['google.service']._get_authorize_uri(cr, uid, from_url, self.STR_SERVICE,
                                                             scope=self.get_calendar_scope(), context=context)
        return url
