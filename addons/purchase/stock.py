...
class stock_move(osv.osv):
    ...
    def _create_invoice_line_from_vals(self, cr, uid, move, invoice_line_vals, context=None):
        ...
        invoice_line_id = super(stock_move, self)._create_invoice_line_from_vals(cr, uid, move, invoice_line_vals,
                                                                                 context=context)
        if context.get('inv_type') in ('in_invoice', 'in_refund') and move.purchase_line_id:
            ...
            purchase_line_ids = purchase_line_obj.search(cr, uid,
                                                         [('order_id', '=', purchase_id), ('invoice_lines', '=', False),
                                                          '|', ('product_id', '=', False),
    ...
    def _get_moves_taxes(self, cr, uid, moves, inv_type, context=None):
        is_extra_move, extra_move_tax = super(stock_move, self)._get_moves_taxes(cr, uid, moves, inv_type,
                                                                                 context=context)
            ...
            for move in moves:
                if move.purchase_line_id:
                    ...
                    extra_move_tax[move.picking_id, move.product_id] = [
                        (6, 0, [x.id for x in move.purchase_line_id.taxes_id])]
                elif move.product_id.product_tmpl_id.supplier_taxes_id:
                    mov_id = self.search(cr, uid,
                                         [('purchase_line_id', '!=', False), ('picking_id', '=', move.picking_id.id)],
                                         limit=1, context=context)
                    if mov_id:
                        mov = self.browse(cr, uid, mov_id[0], context=context)
                        fp = mov.purchase_line_id.order_id.fiscal_position
                        res = self.pool.get("account.invoice.line").product_id_change(cr, uid, [], move.product_id.id,
                                                                                      None,
...
class stock_picking(osv.osv):
    ...
    _columns = {
        'reception_to_invoice': fields.function(_get_to_invoice, type='boolean',
                                                ...
                                                store={
                                                    'stock.move': (
                                                    _get_picking_to_recompute, ['purchase_line_id', 'picking_id'], 10),
                                                }),
    }
...
class stock_warehouse(osv.osv):
    ...
    def _handle_renaming(self, cr, uid, warehouse, name, code, context=None):
        ...
        if warehouse.buy_pull_id:
            pull_obj.write(cr, uid, warehouse.buy_pull_id.id,
                           {'name': warehouse.buy_pull_id.name.replace(warehouse.name, name, 1)}, context=context)
        return res

    def change_route(self, cr, uid, ids, warehouse, new_reception_step=False, new_delivery_step=False, context=None):
        res = super(stock_warehouse, self).change_route(cr, uid, ids, warehouse, new_reception_step=new_reception_step,
                                                        new_delivery_step=new_delivery_step, context=context)
        if warehouse.in_type_id.default_location_dest_id != warehouse.buy_pull_id.location_id:
            self.pool.get('procurement.rule').write(cr, uid, warehouse.buy_pull_id.id,
                                                    {'location_id': warehouse.in_type_id.default_location_dest_id.id},
                                                    context=context)
        return res
