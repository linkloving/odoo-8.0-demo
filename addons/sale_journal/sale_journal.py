...
class sale_journal_invoice_type(osv.osv):
    ...
    _columns = {
        ...
        'active': fields.boolean('Active',
                                 help="If the active field is set to False, it will allow you to hide the invoice type without removing it."),
        'note': fields.text('Note'),
        'invoicing_method': fields.selection([('simple', 'Non grouped'), ('grouped', 'Grouped')], 'Invoicing method',
                                             required=True),
    }
    _defaults = {
        'active': True,
...
class stock_move(osv.osv):
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for proc in procs_to_check:
            pickings = list(
                set([x.picking_id.id for x in proc.move_ids if x.picking_id and not x.picking_id.invoice_type_id]))
            if pickings:
                pick_obj.write(cr, uid, pickings, {'invoice_type_id': proc.sale_line_id.order_id.invoice_type_id.id},
                               context=context)
        return res
...
class sale(osv.osv):
    ...
    _columns = {
        'invoice_type_id': fields.many2one('sale_journal.invoice.type', 'Invoice Type',
                                           help="Generate invoice based on the selected option.")
    }
