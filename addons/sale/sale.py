...
class sale_order(osv.osv):
    ...
    def action_wait(self, cr, uid, ids, context=None):
        ...
        for o in self.browse(cr, uid, ids):
            ...
            if (o.order_policy == 'manual') or noprod:
                ...
                self.write(cr, uid, [o.id], {'state': 'progress',
                                             'date_confirm': fields.date.context_today(self, cr, uid, context=context)})
            self.pool.get('sale.order.line').button_confirm(cr, uid,
                                                            [x.id for x in o.order_line if x.state != 'cancel'])
        return True
        ...
        for order_id in ids:
            ...
            if email_act and email_act.get('context'):
                ...
                composer_values.update(
                    composer_obj.onchange_template_id(cr, uid, None, *template_values, context=context).get('value',
                                                                                                            {}))
    ...
    def procurement_needed(self, cr, uid, ids, context=None):
        ...
        res = []
        for order in self.browse(cr, uid, ids, context=context):
            res.append(sale_line_obj.need_procurement(cr, uid,
                                                      [line.id for line in order.order_line if line.state != 'cancel'],
                                                      context=context))
        return any(res)
        ...
        for line in order_lines:
            ...
            # link all  (6, 0, IDS)
                ...
                for line_id in line_ids:
                    ...
                    if prod and prod.taxes_id:
                        order_line.append([1, line_id, {
                            'tax_id': [[6, 0, fiscal_obj.map_tax(cr, uid, fpos, prod.taxes_id, context=context)]]}])
                    else:
                        order_line.append([4, line_id])
            else:
                order_line.append(line)
        return {
            'value': {'order_line': order_line, 'amount_untaxed': False, 'amount_tax': False, 'amount_total': False}}
...
class sale_order_line(osv.osv):
    ...
    _columns = {
        ...
        'price_reduce': fields.function(_get_price_reduce, type='float', string='Price Reduce',
    ...
    def create(self, cr, uid, values, context=None):
        if values.get('order_id') and values.get('product_id') and any(
                        f not in values for f in ['name', 'price_unit', 'product_uom_qty', 'product_uom']):
    ...
    def product_uom_change(self, cursor, user, ids, pricelist, product, qty=0,
        ...
        return self.product_id_change(cursor, user, ids, pricelist, product,
                                      ...
                                      date_order=date_order, fiscal_position=context.get('fiscal_position', False),
                                      context=context)
