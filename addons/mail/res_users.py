...
class res_users(osv.Model):
    ...
    def _auto_init(self, cr, context=None):
        ...
        return self.pool.get('mail.alias').migrate_to_alias(cr, self._name, self._table,
                                                            super(res_users, self)._auto_init,
                                                            self._name, self._columns['alias_id'], 'login',
                                                            alias_force_key='id', context=context)
    ...
    def create(self, cr, uid, data, context=None):
        ...
        self.pool.get('mail.alias').write(cr, SUPERUSER_ID, [user.alias_id.id],
                                          {"alias_force_thread_id": user_id, "alias_parent_thread_id": user_id},
                                          context)
    ...
    def message_get_partner_info_from_emails(self, cr, uid, emails, link_mail=False, context=None):
        return self.pool.get('mail.thread').message_get_partner_info_from_emails(cr, uid, emails, link_mail=link_mail,
                                                                                 context=context)
