...
class mail_followers(osv.Model):
    ...
    _sql_constraints = [('mail_followers_res_partner_res_model_id_uniq', 'unique(res_model,res_id,partner_id)',
...
class mail_notification(osv.Model):
    ...
    def init(self, cr):
        cr.execute('SELECT indexname FROM pg_indexes WHERE indexname = %s',
                   ('mail_notification_partner_id_read_starred_message_id',))
        if not cr.fetchone():
            cr.execute(
                'CREATE INDEX mail_notification_partner_id_read_starred_message_id ON mail_notification (partner_id, is_read, starred, message_id)')
    ...
    def get_signature_footer(self, cr, uid, user_id, res_model=None, res_id=None, context=None, user_signature=True):
        ...
        if user.company_id.website:
            website_url = ('http://%s' % user.company_id.website) if not user.company_id.website.lower().startswith(
                ('http:', 'https:')) \
    ...
    def update_message_notification(self, cr, uid, ids, message_id, partner_ids, context=None):
        ...
        for new_pid in new_pids:
            new_notif_ids.append(
                self.create(cr, uid, {'message_id': message_id, 'partner_id': new_pid, 'is_read': False},
                            context=context))
    ...
    def _notify_email(self, cr, uid, ids, message_id, force_send=False, user_signature=True, context=None):
        ...
        user_id = message.author_id and message.author_id.user_ids and message.author_id.user_ids[0] and \
                  message.author_id.user_ids[0].id or None
        signature_company = self.get_signature_footer(cr, uid, user_id, res_model=message.model, res_id=message.res_id,
                                                      context=context,
        ...
        # custom values
        custom_values = dict()
        if message.model and message.res_id and self.pool.get(message.model) and hasattr(self.pool[message.model],
                                                                                         'message_get_email_values'):
            custom_values = self.pool[message.model].message_get_email_values(cr, uid, message.res_id, message,
                                                                              context=context)
        ...
        for chunk in chunks:
            if message.model and message.res_id and self.pool.get(message.model) and hasattr(self.pool[message.model],
                ...
                recipient_values = self.pool[message.model].message_get_recipient_values(cr, uid, message.res_id,
                                                                                         notif_message=message,
                                                                                         recipient_ids=chunk,
                                                                                         context=context)
            else:
                recipient_values = self.pool['mail.thread'].message_get_recipient_values(cr, uid, message.res_id,
                                                                                         notif_message=message,
                                                                                         recipient_ids=chunk,
                                                                                         context=context)
            mail_values = {
    ...
    def _notify(self, cr, uid, message_id, partners_to_notify=None, context=None,
        ...
        notif_ids = self.search(cr, SUPERUSER_ID,
                                [('message_id', '=', message_id), ('partner_id', 'in', partners_to_notify)],
                                context=context)

        # update or create notifications
        new_notif_ids = self.update_message_notification(cr, SUPERUSER_ID, notif_ids, message_id, partners_to_notify,
                                                         context=context)
