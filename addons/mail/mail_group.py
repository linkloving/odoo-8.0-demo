...
class mail_group(osv.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        self.pool.get('mail.alias').write(cr, uid, [group.alias_id.id], {"alias_force_thread_id": mail_group_id,
                                                                         'alias_parent_thread_id': mail_group_id},
                                          context)
        group = self.browse(cr, uid, mail_group_id, context=context)
        ...
        if ref:
            ...
            newref = cobj.copy(cr, SUPERUSER_ID, ref[1], default={'params': str(params), 'name': vals['name']},
                               context=context)
            mobj.write(cr, SUPERUSER_ID, menu_id,
                       {'action': 'ir.actions.client,' + str(newref), 'mail_group_id': mail_group_id}, context=context)
    ...
    def message_get_recipient_values(self, cr, uid, id, notif_message=None, recipient_ids=None, context=None):
        ...
        if group.alias_domain and group.alias_name:
            return {
                'email_to': ','.join(formataddr((partner.name, partner.email)) for partner in
                                     self.pool['res.partner'].browse(cr, SUPERUSER_ID, recipient_ids, context=context)),
                'recipient_ids': [],
            }
        return super(mail_group, self).message_get_recipient_values(cr, uid, id, notif_message=notif_message,
