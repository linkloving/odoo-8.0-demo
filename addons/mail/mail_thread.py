...
class mail_thread(osv.AbstractModel):
    ...
    def get_empty_list_help(self, cr, uid, help, context=None):
        ...
        if not alias and catchall_domain and model:  # no res_id or res_id not linked to an alias -> generic help message, take a generic alias of the model
            ...
            alias_ids = alias_obj.search(cr, uid,
                                         [("alias_parent_model_id.model", "=", model), ("alias_name", "!=", False),
                                          ('alias_force_thread_id', '=', False),
                                          ('alias_parent_thread_id', '=', False)], context=context, order='id ASC')
    ...
    def _get_message_data(self, cr, uid, ids, name, args, context=None):
        ...
        for id in ids:
            if res[id]['message_unread_count']:
                title = res[id]['message_unread_count'] > 1 and _("You have %d unread messages") % res[id][
                    'message_unread_count'] or _("You have one unread message")
                res[id][
                    'message_summary'] = "<span class='oe_kanban_mail_new' title='%s'><span class='oe_e'>9</span> %d %s</span>" % (
                title, res[id].pop('message_unread_count'), _("New"))
    ...
    def _get_subscription_data(self, cr, uid, ids, name, args, user_pid=None, context=None):
        ...
        if user_pid is None:
            user_pid = \
            self.pool.get('res.users').read(cr, uid, [uid], ['partner_id'], context=context)[0]['partner_id'][0]
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        res = super(mail_thread, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context,
        ...
        for browse_record in self.browse(cr, uid, ids, context=context):
            ...
            for subtype in subtypes:
                ...
                if not (subtype_rec and subtype_rec.exists()):
                    ...
                    continue
                message = format_message(subtype_rec.description if subtype_rec.description else subtype_rec.name,
                                         tracked_values)
    ...
    def message_redirect_action(self, cr, uid, context=None):
        ...
        act_model, act_id = self.pool.get('ir.model.data').get_object_reference(cr, uid,
                                                                                *self._get_inbox_action_xml_id(cr, uid,
                                                                                                               context=context))
    ...
    def message_get_default_recipients(self, cr, uid, ids, context=None):
        if context and context.get('thread_model') and context['thread_model'] in self.pool and context[
            ...
            if model_name and model_name != 'mail.thread':
                ...
                doc_names.update(
                    dict((ng_res[0], ng_res[1])
                         for ng_res in
                         self.pool[model_name].name_get(cr, SUPERUSER_ID, aliases.keys(), context=context)))
            # left ids: use catchall
            left_ids = set(ids).difference(set(aliases.keys()))
            if left_ids:
                catchall_alias = self.pool['ir.config_parameter'].get_param(cr, uid, "mail.catchall.alias",
                                                                            context=context)
                if catchall_alias:
    ...
    def message_route_verify(self, cr, uid, message, message_dict, route, update_author=True, assert_model=True,
                             create_fallback=True, allow_private=False, context=None):
        ...
        if not model and thread_id:
            if assert_model:
                if thread_id:
                    raise ValueError(
                        'Routing: posting a message without model should be with a null res_id (private message), received %s.' % thread_id)
            _warn(
                'posting a message without model should be with a null res_id (private message), received %s resetting thread_id' % thread_id)
            thread_id = 0
        ...
        if thread_id and not model_pool.exists(cr, uid, thread_id):
            if create_fallback:
                ...
                assert model_pool.exists(cr, uid, thread_id), 'Routing: reply to missing document (%s,%s)' % (
                model, thread_id)
            else:
        ...
        if thread_id and model and not hasattr(model_pool, 'message_update'):
            if create_fallback:
                ...
                assert hasattr(model_pool,
                               'message_update'), 'Routing: model %s does not accept document update, crashing' % model
            else:
        ...
        if alias and alias.alias_contact == 'followers' and (thread_id or alias.alias_parent_thread_id):
            if thread_id:
                ...
                obj = self.pool[alias.alias_parent_model_id.model].browse(cr, uid, alias.alias_parent_thread_id,
                                                                          context=context)
        ...
        if ref_match:
            ...
            if local_hostname == reply_hostname:
                ...
                if thread_id and model in self.pool:
                    ...
                    if compat_mail_msg_ids and model_obj.exists(cr, uid, thread_id) and hasattr(model_obj,
                                                                                                'message_update'):
                        route = self.message_route_verify(
                            cr, uid, message, message_dict,
        ...
        if local_parts:
            ...
            if alias_ids:
                ...
                for alias in mail_alias.browse(cr, uid, alias_ids, context=context):
                    ...
                    route = (
                    alias.alias_model_id.model, alias.alias_force_thread_id, eval(alias.alias_defaults), user_id, alias)
    ...
    def message_route_process(self, cr, uid, message, message_dict, routes, context=None):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            new_msg_id = model_pool.message_post(cr, uid, [thread_id], context=context, subtype='mail.mt_comment',
                                                 **message_dict)
    ...
    def message_parse(self, cr, uid, message, save_original=False, context=None):
        ...
        if message.get('In-Reply-To'):
            parent_ids = self.pool.get('mail.message').search(cr, uid, [
        ...
        if message.get('References') and 'parent_id' not in msg_dict:
            ...
            parent_ids = self.pool.get('mail.message').search(cr, uid,
                                                              [('message_id', 'in', [x.strip() for x in msg_list])])
            if parent_ids:
                msg_dict['parent_id'] = parent_ids[0]
    ...
    def _message_add_suggested_recipient(self, cr, uid, result, obj, partner=None, email=None, reason='', context=None):
        ...
        if email and not partner:
            ...
            if partner_info.get('partner_id'):
                partner = \
                self.pool.get('res.partner').browse(cr, SUPERUSER_ID, [partner_info['partner_id']], context=context)[0]
    ...
    def message_get_suggested_recipients(self, cr, uid, ids, context=None):
        ...
        if 'user_id' in self._fields:
            for obj in self.browse(cr, SUPERUSER_ID, ids,
                                   context=context):  # SUPERUSER because of a read on res.users that would crash otherwise
                if not obj.user_id or not obj.user_id.partner_id:
                    continue
                self._message_add_suggested_recipient(cr, uid, result, obj, partner=obj.user_id.partner_id,
                                                      reason=self._fields['user_id'].string, context=context)
        return result
    ...
    def message_partner_info_from_emails(self, cr, uid, id, emails, link_mail=False, context=None):
        ...
        for idx in range(len(emails)):
            ...
            if link_mail and partner_info['partner_id']:
                ...
                if message_ids:
                    mail_message_obj.write(cr, SUPERUSER_ID, message_ids, {'author_id': partner_info['partner_id']},
                                           context=context)
        return result

    def _message_preprocess_attachments(self, cr, uid, attachments, attachment_ids, attach_model, attach_res_id,
                                        context=None):
        ...
        if attachment_ids:
            ...
            if filtered_attachment_ids:
                Attachment.write(cr, SUPERUSER_ID, filtered_attachment_ids,
                                 {'res_model': attach_model, 'res_id': attach_res_id}, context=context)
    ...
    def get_suggested_thread(self, cr, uid, removed_suggested_threads=None, context=None):
        ...
        thread_ids = self.search(cr, uid,
                                 [('id', 'not in', removed_suggested_threads), ('message_is_follower', '=', False)],
                                 context=context)
    ...
    def message_change_thread(self, cr, uid, id, new_res_id, new_model, context=None):
        ...
        subtype_res_id = self.pool.get('ir.model.data').xmlid_to_res_id(cr, uid, 'mail.mt_comment',
        ...
        message_obj.write(cr, uid, msg_ids_not_comment, {"res_id": new_res_id, "model": new_model, "subtype_id": None},
                          context=context)
