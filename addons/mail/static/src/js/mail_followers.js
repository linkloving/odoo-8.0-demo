openerp_mail_followers = function (session, mail) {
    ...
    mail_followers.Followers = session.web.form.AbstractField.extend({
        ...
        display_followers: function (records) {
            ...
            self.message_is_follower = _.indexOf(this.followers.map(function (rec) {
                    return rec[2]['is_uid']
                }), true) != -1;
            ...
            _(truncated).each(function (record) {
                ...
                $(session.web.qweb.render('mail.followers.partner', {
                    'record': partner,
                    'widget': self
        ...
        fetch_subtypes: function (user_pid) {
            ...
            this.ds_model.call('message_get_subscription_data', [[id], user_pid, new session.web.CompoundContext(this.build_context(), {})])
                .then(function (data) {
                    self.display_subtypes(data, id, dialog);
                });
        },
        ...
        display_subtypes: function (data, id, dialog) {
            ...
            this.records_length = $.map(records, function (value, index) {
                return index;
            }).length;
            ...
            _(records).each(function (record, record_name) {
                ...
                $(session.web.qweb.render('mail.followers.subtype', {
                    'record': record,
                    'dialog': dialog
                })).appendTo($list);
            });
