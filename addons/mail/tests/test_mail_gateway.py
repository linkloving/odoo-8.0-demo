...
class TestMailgateway(TestMail):
    ...
    def test_10_message_process(self):
        ...
        self.assertEqual(len(frog_group.message_follower_ids), 0,
        ...
        self.assertIn(formataddr(('Sylvie Lelitre', 'test.sylvie.lelitre@agrolait.com')),
                      sent_emails[0].get('email_to'),
        ...
        self.mail_group.write(cr, uid, [frog_group.id], {'alias_name': 'frogs', 'alias_contact': 'followers',
                                                         'alias_force_thread_id': frog_group.id})
        ...
        frog_groups = format_and_process(MAIL_TEMPLATE, email_from='other5@gmail.com',
                                         ...
                                         to='noone@example.com>', subject='spam',
                                         extra='In-Reply-To: <12321321-openerp-%d-mail.group@%s>' % (
                                         frog_group.id, socket.gethostname()))
        ...
        frog_group = self.mail_group.browse(cr, uid, frog_groups[0])
        # Test: no new message
        self.assertEqual(len(frog_group.message_ids), 4,
                         'message_process: message with already existing message_id should not have been duplicated')
        # Test: message_id is still unique
        msg_ids = self.mail_message.search(cr, uid, [
            ('message_id', 'ilike', '<1198923581.41972151344608186760.JavaMail.diff1@agrolait.com>')])
