...
class TestMailMail(TestMail):
    def test_00_partner_find_from_email(self):
        ...
        partner_info = self.mail_thread.message_partner_info_from_emails(cr, uid, None, ['Maybe Raoul <test@test.fr>'],
                                                                         link_mail=False)[0]
        ...
        # 2 - User BRaoul
        p_b_id = self.res_partner.create(cr, uid,
                                         {'name': 'BRaoul', 'email': 'test@test.fr', 'user_ids': [(4, user_raoul.id)]})
        ...
        partner_info = \
        self.mail_group.message_partner_info_from_emails(cr, uid, group_pigs.id, ['Maybe Raoul <test@test.fr>'],
                                                         link_mail=False)[0]
        self.assertEqual(partner_info['partner_id'], p_b_id,
                         'mail_thread: message_partner_info_from_emails wrong partner found')
...
class TestMailMessage(TestMail):
    def test_00_mail_message_values(self):
        ...
        msg_id = self.mail_message.create(cr, user_raoul_id,
                                          {'no_auto_thread': True, 'reply_to': reply_to1, 'email_from': email_from1})
        ...
        mail_id = self.mail_mail.create(cr, user_raoul_id,
                                        {'mail_message_id': msg_id, 'state': 'cancel', 'reply_to': reply_to2})
        ...
        self.registry('ir.config_parameter').unlink(cr, uid, self.registry('ir.config_parameter').search(cr, uid, [
    ...
    def test_10_mail_message_search_access_rights(self):
        ...
        msg_id2 = self.mail_message.create(cr, uid, {'subject': '_Test', 'body': 'A+B',
                                                     'partner_ids': [(6, 0, [self.partner_bert_id])],
                                                     'subtype_id': subtype_id})
        msg_id3 = self.mail_message.create(cr, uid, {'subject': '_Test', 'body': 'A Pigs', 'model': 'mail.group',
                                                     'res_id': group_pigs_id, 'subtype_id': subtype_id})
        msg_id4 = self.mail_message.create(cr, uid, {'subject': '_Test', 'body': 'A+B Pigs', 'model': 'mail.group',
                                                     'res_id': group_pigs_id,
                                                     'partner_ids': [(6, 0, [self.partner_bert_id])],
                                                     'subtype_id': subtype_id})
        msg_id5 = self.mail_message.create(cr, uid, {'subject': '_Test', 'body': 'A+R Pigs', 'model': 'mail.group',
                                                     'res_id': group_pigs_id,
                                                     'partner_ids': [(6, 0, [self.partner_raoul_id])],
                                                     'subtype_id': subtype_id})
        msg_id6 = self.mail_message.create(cr, uid, {'subject': '_Test', 'body': 'A Birds', 'model': 'mail.group',
                                                     'res_id': group_birds_id, 'subtype_id': subtype_id})
        msg_id7 = self.mail_message.create(cr, self.user_raoul_id,
                                           {'subject': '_Test', 'body': 'B', 'subtype_id': subtype_id})
        msg_id8 = self.mail_message.create(cr, self.user_raoul_id, {'subject': '_Test', 'body': 'B+R',
                                                                    'partner_ids': [(6, 0, [self.partner_raoul_id])],
                                                                    'subtype_id': subtype_id})
        ...
        msg_ids = self.mail_message.search(cr, self.user_raoul_id,
                                           [('subject', 'like', '_Test'), ('body', 'like', 'A')])
        ...
        self.assertEqual(set([msg_id1, msg_id2, msg_id3, msg_id4, msg_id5, msg_id6, msg_id7, msg_id8]), set(msg_ids),
                         'mail_message search failed')
    ...
    def test_15_mail_message_check_access_rule(self):
        ...
        attachment_id = self.ir_attachment.create(cr, uid,
                                                  {'datas': 'My attachment'.encode('base64'), 'name': 'doc.txt',
                                                   'datas_fname': 'doc.txt'})
        ...
        with self.assertRaises(AccessError):
            self.mail_message.create(cr, user_bert_id,
                                     {'model': 'mail.group', 'res_id': self.group_pigs_id, 'body': 'Test'})
        # Do: Bert create a message on Jobs -> ko, no creation rights
        with self.assertRaises(AccessError):
            self.mail_message.create(cr, user_bert_id,
                                     {'model': 'mail.group', 'res_id': self.group_jobs_id, 'body': 'Test'})
        ...
        self.mail_message.create(cr, user_raoul_id,
                                 {'model': 'mail.group', 'res_id': self.group_jobs_id, 'body': 'Test'})
        # Do: Raoul creates a message on Priv -> ko, no write access to the related document
        with self.assertRaises(except_orm):
            self.mail_message.create(cr, user_raoul_id,
                                     {'model': 'mail.group', 'res_id': self.group_priv_id, 'body': 'Test'})
    ...
    def test_20_message_set_star(self):
        ...
        notif_ids = self.mail_notification.search(cr, uid, [('partner_id', '=', self.partner_admin_id),
                                                            ('message_id', '=', msg.id)])
        ...
        msg_raoul.refresh()
        # Test: notification exists
        notif_ids = self.mail_notification.search(cr, uid, [('partner_id', '=', self.partner_raoul_id),
                                                            ('message_id', '=', msg.id)])
        self.assertEqual(len(notif_ids), 1, 'mail_message set_message_starred: more than one notification created')
        # Test: notification starred
        notif = self.mail_notification.browse(cr, uid, notif_ids[0])
    ...
    def test_40_message_vote(self):
        ...
        self.assertEqual(set(msg.vote_user_ids), set([self.user_admin]),
                         'mail_message vote: after voting, Admin should be in the voter')
        # Do: Bert vote for msg
        self.mail_message.vote_toggle(cr, self.user_raoul_id, [msg.id])
        msg_raoul.refresh()
        # Test: msg has Admin and Bert as voters
        self.assertEqual(set(msg_raoul.vote_user_ids), set([self.user_admin, self.user_raoul]),
                         'mail_message vote: after voting, Admin and Bert should be in the voters')
        # Do: Admin unvote for msg
        self.mail_message.vote_toggle(cr, uid, [msg.id])
        msg.refresh()
        msg_raoul.refresh()
        # Test: msg has Bert as voter
        self.assertEqual(set(msg.vote_user_ids), set([self.user_raoul]),
                         'mail_message vote: after unvoting, Bert should be in the voter')
        self.assertEqual(set(msg_raoul.vote_user_ids), set([self.user_raoul]),
                         'mail_message vote: after unvoting, Bert should be in the voter')
    ...
    def test_50_mail_flow_access_rights(self):
        ...
        # Prepare groups: Pigs (employee), Jobs (public)
        pigs_msg_id = self.mail_group.message_post(cr, uid, self.group_pigs_id, body='Message',
                                                   partner_ids=[self.partner_admin_id])
        jobs_msg_id = self.mail_group.message_post(cr, uid, self.group_jobs_id, body='Message',
                                                   partner_ids=[self.partner_admin_id])
        ...
        jobs_message_ids = self.mail_group.read(cr, user_bert_id, [self.group_jobs_id], ['message_ids'])[0][
            'message_ids']
        ...
        compose_id = mail_compose.create(cr, user_raoul_id,
                                         ...
                                         {'default_composition_mode': 'comment', 'default_model': 'mail.group',
                                          'default_res_id': self.group_jobs_id})
