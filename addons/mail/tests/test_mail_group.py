...
class TestMailGroup(TestMail):
    ...
    def tearDownClass(cls):
        ...
        def mail_group_message_get_recipient_values(self, cr, uid, id, notif_message=None, recipient_ids=None,
                                                    context=None):
            return self.pool['mail.thread'].message_get_recipient_values(cr, uid, id, notif_message=notif_message,
                                                                         recipient_ids=recipient_ids, context=context)

        cls.registry('mail.group')._patch_method('message_get_recipient_values',
                                                 mail_group_message_get_recipient_values)
    ...
    def test_00_mail_group_access_rights(self):
        ...
        fol_ids = self.mail_followers.search(cr, uid,
                                             [('res_model', '=', 'mail.group'), ('res_id', '=', self.group_priv_id)])
    ...
    def test_mail_group_notification_recipients_grouped(self):
        ...
        self.mail_group.message_post(self.cr, self.uid, [self.group_pigs_id], body="Test", type='comment',
                                     subtype='mt_comment')
        ...
        for email in sent_emails:
            self.assertEqual(
                set(email['email_to']),
                set([formataddr((self.user_raoul.name, self.user_raoul.email)),
                     formataddr((self.user_bert.name, self.user_bert.email))]))
    ...
    def test_mail_group_notification_recipients_separated(self):
        ...
        for email in sent_emails:
            self.assertIn(
                email['email_to'][0],
                [formataddr((self.user_raoul.name, self.user_raoul.email)),
                 formataddr((self.user_bert.name, self.user_bert.email))])
