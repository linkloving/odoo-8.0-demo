...
class mail_alias(osv.Model):
    ...
    def migrate_to_alias(self, cr, child_model_name, child_table_name, child_model_auto_init_fct,
                         alias_model_name, alias_id_column, alias_key, alias_prefix='', alias_force_key='',
                         alias_defaults={},
        ...
        for obj_data in child_class_model.read(cr, SUPERUSER_ID, no_alias_ids, [alias_key]):
            ...
            alias_create_ctx = dict(context, alias_model_name=alias_model_name,
                                    alias_parent_model_name=child_model_name)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if parent_model_name:
            model_id = self.pool.get('ir.model').search(cr, uid, [('model', '=', parent_model_name)], context=context)[
                0]
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('alias_name') and ids:
            vals['alias_name'] = self._clean_and_make_unique(cr, uid, vals.get('alias_name'), alias_id=ids[0],
                                                             context=context)
