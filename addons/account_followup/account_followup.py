...
class followup_line(osv.osv):
    def _get_default_template(self, cr, uid, ids, context=None):
        try:
            return self.pool.get('ir.model.data').get_object_reference(cr, uid, 'account_followup',
...
class res_partner(osv.osv):
    ...
    def _get_latest(self, cr, uid, ids, names, arg, context=None, company_id=None):
        ...
        for partner in self.browse(cr, uid, ids, context=context):
            ...
            for aml in amls:
                if (aml.company_id == company) and (aml.followup_line_id != False) and (
                    not latest_days or latest_days < aml.followup_line_id.delay):
    ...
    def do_partner_print(self, cr, uid, wizard_partner_ids, data, context=None):
        ...
        return self.pool['report'].get_action(cr, uid, [], 'account_followup.report_followup', data=datas,
                                              context=context)
            ...
            if partners_to_email:
                ...
                if partner not in partners_to_email:
                    self.message_post(cr, uid, [partner.id], body=_('Overdue email sent to %s' % ', '.join(
                        ['%s <%s>' % (partner.name, partner.email) for partner in partners_to_email])), context=context)
            else:
                unknown_mails = unknown_mails + 1
                action_text = _("Email not sent because of email address of partner not filled in")
                if partner.payment_next_action_date:
                    payment_action_date = min(fields.date.context_today(self, cr, uid, context=ctx),
            ...
            for currency_dict in final_res:
                ...
                for aml in currency_dict['line']:
                    ...
                    if date <= current_date and aml['balance'] > 0:
                        strbegin = "<TD><B>"
                        strend = "</B></TD>"
                    followup_table += "<TR>" + strbegin + str(aml['date']) + strend + strbegin + aml[
                        'name'] + strend + strbegin + (aml['ref'] or '') + strend + strbegin + str(
                        date) + strend + strbegin + str(aml['balance']) + strend + strbegin + block + strend + "</TR>"
        ...
        if vals.get("payment_responsible_id", False):
            for part in self.browse(cr, uid, ids, context=context):
                if part.payment_responsible_id <> vals["payment_responsible_id"]:
                    ...
                    responsible_partner_id = self.pool.get("res.users").browse(cr, uid, vals['payment_responsible_id'],
                                                                               context=context).partner_id.id
                    self.pool.get("mail.thread").message_post(cr, uid, 0,
                                                              body=_(
                                                                  "You became responsible to do the next action for the payment follow-up of") + " <b><a href='#id=" + str(
                                                                  part.id) + "&view_type=form&model=res.partner'> " + part.name + " </a></b>",
                                                              type='comment',
    ...
    def action_done(self, cr, uid, ids, context=None):
        return self.write(cr, uid, ids, {'payment_next_action_date': False, 'payment_next_action': '',
    ...
    def do_button_print(self, cr, uid, ids, context=None):
        ...
        followup_ids = self.pool.get('account_followup.followup').search(cr, uid, [('company_id', '=', company_id)],
                                                                         context=context)
        if not followup_ids:
    ...
    _columns = {
        ...
        'latest_followup_level_id': fields.function(_get_latest, method=True,
                                                    type='many2one', relation='account_followup.followup.line',
        ...
        'latest_followup_level_id_without_lit': fields.function(_get_latest, method=True,
                                                                ...
                                                                store={
                                                                    ...
                                                                    'account.move.line': (
                                                                    _get_partners, ['followup_line_id'], 10),
                                                                },
                                                                multi="latest"),
