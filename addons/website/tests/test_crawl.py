...
class Crawler(openerp.tests.HttpCase):
    ...
    def test_10_crawl_public(self):
        ...
        _logger.log(25, "public crawled %s urls in %.2fs %s queries, %.3fs %.2fq per request, ", count, duration, sql,
    ...
    def test_20_crawl_demo(self):
        ...
        duration = time.time() - t0
        sql = self.registry.test_cr.sql_log_count - t0_sql
        _logger.log(25, "demo crawled %s urls in %.2fs %s queries, %.3fs %.2fq per request", count, duration, sql,
                    duration / count, float(sql) / count)
    ...
    def test_30_crawl_admin(self):
        ...
        duration = time.time() - t0
        sql = self.registry.test_cr.sql_log_count - t0_sql
        _logger.log(25, "admin crawled %s urls in %.2fs %s queries, %.3fs %.2fq per request", count, duration, sql,
                    duration / count, float(sql) / count)
