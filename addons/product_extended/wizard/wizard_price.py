...
class wizard_price(osv.osv):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        res['info_field'] = str(
            product_pool.compute_price(cr, uid, [], template_ids=[product_obj.id], test=True, context=context))
        return res
    ...
    def compute_from_bom(self, cr, uid, ids, context=None):
        ...
        if model != 'product.template':
            raise except_orm(_('Wrong model!'), _(
                'This wizard is build for product templates, while you are currently running it from a product variant.'))
        rec_id = context and context.get('active_id', False)
        assert rec_id, _('Active ID is not set in Context.')
        prod_obj = self.pool.get('product.template')
        res = self.browse(cr, uid, ids, context=context)
        prod = prod_obj.browse(cr, uid, rec_id, context=context)
        prod_obj.compute_price(cr, uid, [], template_ids=[prod.id], real_time_accounting=res[0].real_time_accounting,
