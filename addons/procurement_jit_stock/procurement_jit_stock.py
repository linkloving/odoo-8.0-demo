...
class procurement_order(osv.osv):
    ...
    def run(self, cr, uid, ids, autocommit=False, context=None):
        ...
        procurement_ids = self.search(cr, uid, [('move_dest_id.procurement_id', 'in', ids),
                                                ('state', 'not in', ['exception', 'cancel'])], order='id',
                                      context=context)
...
class stock_move(osv.osv):
    ...
    def _create_procurements(self, cr, uid, moves, context=None):
        res = super(stock_move, self)._create_procurements(cr, uid, moves,
                                                           context=dict(context or {}, procurement_autorun_defer=True))
