...
class price_type(osv.osv):
    ...
    def _price_field_get(self, cr, uid, context=None):
        ...
        ids = mf.search(cr, uid,
                        [('model', 'in', (('product.product'), ('product.template'))), ('ttype', '=', 'float')],
                        context=context)
        res = []
    ...
    _columns = {
        ...
        "field": fields.selection(_price_field_get, "Product Field", size=32, required=True,
...
class product_pricelist(osv.osv):
    ...
    def price_get_multi(self, cr, uid, ids, products_by_qty_by_partner, context=None):
        return dict((key, dict((key, price[0]) for key, price in value.items())) for key, value in
                    self.price_rule_get_multi(cr, uid, ids, products_by_qty_by_partner, context=context).items())
    ...
    def _price_get_multi(self, cr, uid, pricelist, products_by_qty_by_partner, context=None):
        return dict((key, price[0]) for key, price in
                    self._price_rule_get_multi(cr, uid, pricelist, products_by_qty_by_partner, context=context).items())
    ...
    def price_get(self, cr, uid, ids, prod_id, qty, partner=None, context=None):
        return dict((key, price[0]) for key, price in
                    self.price_rule_get(cr, uid, ids, prod_id, qty, partner=partner, context=context).items())
...
class product_pricelist_item(osv.osv):
    ...
    def _get_default_base(self, cr, uid, fields, context=None):
        ...
        if fields.get('type') == 'purchase':
            product_price_type_ids = product_price_type_obj.search(cr, uid, [('field', '=', 'standard_price')],
                                                                   context=context)
        elif fields.get('type') == 'sale':
            product_price_type_ids = product_price_type_obj.search(cr, uid, [('field', '=', 'list_price')],
                                                                   context=context)
        else:
            return -1
    ...
    _constraints = [
        (_check_recursion, 'Error! You cannot assign the Main Pricelist as Other Pricelist in PriceList Item!',
         ['base_pricelist_id']),
        (_check_margin, 'Error! The minimum margin should be lower than the maximum margin.',
