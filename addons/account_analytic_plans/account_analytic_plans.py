    ...
    def get(self, cr, obj, ids, name, user=None, offset=0, context=None, values=None):
        ...
        if 'journal_id' in context:
            ...
            if plan and len(plan.plan_ids) > pnum:
                ...
                ids2 = obj.pool[self._obj].search(cr, user, [(self._fields_id, 'in', ids),
                                                             ('analytic_account_id', 'child_of', [acc_id])],
                                                  limit=self._limit)
        if ids2 is None:
...
class account_analytic_line(osv.osv):
    ...
    _columns = {
        'amount_currency': fields.function(_get_amount, string="Amount Currency", type="float", store=True,
...
class account_analytic_plan_line(osv.osv):
    ...
    _columns = {
        ...
        'root_analytic_id': fields.many2one('account.analytic.account', 'Root Account',
                                            help="Root account of this plan.", required=False),
...
class account_analytic_plan_instance(osv.osv):
    ...
    def search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False):
        ...
        res = super(account_analytic_plan_instance, self).search(cr, user, args, offset=offset, limit=limit,
                                                                 order=order,
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if context is None:
            context = {}
        wiz_id = self.pool.get('ir.actions.act_window').search(cr, uid,
                                                               [("name", "=", "analytic.plan.create.model.action")],
                                                               context=context)
        res = super(account_analytic_plan_instance, self).fields_view_get(cr, uid, view_id, view_type, context=context,
    ...
    """ % (tools.to_xml(plan_id.name), wiz_id[0])
                ...
                <newline/>""" % (i, tools.to_xml(line.name), tools.to_xml(line.name),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context and context.get('journal_id'):
            ...
            pids = ana_plan_instance_obj.search(cr, uid, [('name', '=', vals['name']), ('code', '=', vals['code']),
                                                          ('plan_id', '<>', False)], context=context)
            if pids:
            ...
            for i in res:
                ...
                temp_list = ['account1_ids', 'account2_ids', 'account3_ids', 'account4_ids', 'account5_ids',
                             'account6_ids']
                for l in temp_list:
                    if vals.has_key(l):
                        for tempo in vals[l]:
                            if acct_anal_acct.search(cr, uid, [('parent_id', 'child_of', [item.root_analytic_id.id]),
                                                               ('id', '=', tempo[2]['analytic_account_id'])],
                                                     context=context):
                                total_per_plan += tempo[2]['rate']
                if total_per_plan < item.min_required or total_per_plan > item.max_required:
                    raise osv.except_osv(_('Error!'), _('The total should be between %s and %s.') % (
                    str(item.min_required), str(item.max_required)))
...
class account_invoice_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, product, uom_id, qty=0, name='', type='out_invoice', partner_id=False,
                          fposition_id=False, price_unit=False, currency_id=False, company_id=None, context=None):
        res_prod = super(account_invoice_line, self).product_id_change(cr, uid, ids, product, uom_id, qty, name, type,
...
class account_move_line(osv.osv):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        if context is None:
            context = {}
        result = super(account_move_line, self).fields_view_get(cr, uid, view_id, view_type, context=context,
                                                                toolbar=toolbar, submenu=submenu)
        return result
    ...
    def _get_analytic_lines(self, cr, uid, ids, context=None):
        ...
        for il in iml:
            if il.get('analytics_id', False):
                ...
                amount_calc = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, il['price'],
                                              context=ctx) * sign
                qty = il['quantity']
                il['analytic_lines'] = []
...
class account_bank_statement(osv.osv):
    ...
    def _prepare_bank_move_line(self, cr, uid, st_line, move_id, amount, company_currency_id, context=None):
        result = super(account_bank_statement, self)._prepare_bank_move_line(cr, uid, st_line,
                                                                             move_id, amount, company_currency_id,
                                                                             context=context)
