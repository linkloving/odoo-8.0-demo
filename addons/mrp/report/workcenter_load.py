...
class report_custom(report_int):
    def _compute_dates(self, time_unit, start, stop):
        ...
        if time_unit == 'month':
            ...
            dates = {}
            start_week = date(int(start.split("-")[0]), int(start.split("-")[1]),
                              int(start.split("-")[2])).isocalendar()
    ...
    def create(self, cr, uid, ids, datas, context=None):
        ...
        for date in dates_list:
            ...
            for workcenter in workcenters:
                cr.execute("SELECT SUM(mrp_production_workcenter_line.hour) AS hours, SUM(mrp_production_workcenter_line.cycle) AS cycles, \
                           ...
                           (workcenter['id'], dates[date]['start'] + ' 00:00:00', dates[date]['stop'] + ' 23:59:59'))
                res = cr.dictfetchall()
                if not res:
                    vals.append(0.0)
                else:
        ...
        for workcenter in workcenters:
            ...
            ar.add_plot(bar_plot.T(label=workcenter['name'], data=data, fill_style=f, hcol=workcenter_num + 1,
