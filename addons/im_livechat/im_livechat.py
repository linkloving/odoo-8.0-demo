...
class im_livechat_channel(osv.Model):
    ...
    def _script_external(self, cr, uid, ids, name, arg, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            res[record.id] = self.pool['ir.ui.view'].render(cr, uid, 'im_livechat.external_loader', values,
                                                            context=context)
        return res
    ...
    def _script_internal(self, cr, uid, ids, name, arg, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            values["channel"] = record.id
            res[record.id] = self.pool['ir.ui.view'].render(cr, uid, 'im_livechat.internal_loader', values,
                                                            context=context)
        return res
    ...
    _columns = {
        ...
        'user_ids': fields.many2many('res.users', 'im_livechat_channel_im_user', 'channel_id', 'user_id',
                                     string="Users"),
        'are_you_inside': fields.function(_are_you_inside, type='boolean', string='Are you inside the matrix?',
                                          store=False),
        ...
        'default_message': fields.char(string="Welcome Message",
    ...
    def get_channel_session(self, cr, uid, channel_id, anonymous_name, context=None):
        ...
        newid = Session.create(cr, uid,
                               {'user_ids': [(4, user_id)], 'channel_id': channel_id, 'anonymous_name': anonymous_name},
                               context=context)
...
class LiveChatController(http.Controller):
    ...
    def support_page(self, dbname, channel_id, **kwargs):
        registry, cr, uid, context = openerp.modules.registry.RegistryManager.get(
            dbname), request.cr, openerp.SUPERUSER_ID, request.context
        info = registry.get('im_livechat.channel').get_info_for_chat_src(cr, uid, channel_id)
        info["dbname"] = dbname
        info["channel"] = channel_id
        info["channel_name"] = registry.get('im_livechat.channel').read(cr, uid, channel_id, ['name'], context=context)[
            "name"]
