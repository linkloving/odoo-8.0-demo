...
class subscription_subscription(osv.osv):
    ...
    def set_process(self, cr, uid, ids, context=None):
        for row in self.read(cr, uid, ids, context=context):
            mapping = {'name': 'name', 'interval_number': 'interval_number', 'interval_type': 'interval_type',
                       'exec_init': 'numbercall', 'date_init': 'nextcall'}
            res = {'model': 'subscription.subscription', 'args': repr([[row['id']]]), 'function': 'model_copy',
                   'priority': 6, 'user_id': row['user_id'] and row['user_id'][0]}
    ...
    def model_copy(self, cr, uid, ids, context=None):
        for row in self.read(cr, uid, ids, context=context):
            ...
            try:
                ...
                raise osv.except_osv(_('Wrong Source Document!'),
