...
class DocIndex(indexer):
    ...
    def _doIndexFile(self, fname):
        try:
            ...
            _logger.warning(
                "Failed attempt to execute antiword (MS Word reader). Antiword is necessary to index the file %s of MIME type %s. Detailed error available at DEBUG level.",
                fname, self._getMimeTypes()[0])
