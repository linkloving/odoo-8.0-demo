...
class analytic_user_funct_grid(osv.osv):
    ...
    _columns = {
        ...
        'uom_id': fields.related("product_id", "uom_id", relation="product.uom", string="Unit of Measure",
                                 type="many2one", readonly=True),
        'price': fields.float('Price', digits_compute=dp.get_precision('Product Price'),
                              help="Price per hour for this user.", required=True),
    }
...
class hr_analytic_timesheet(osv.osv):
    ...
    def _get_related_user_account_recursiv(self, cr, uid, user_id, account_id):
        temp = self.pool.get('analytic.user.funct.grid').search(cr, uid, [('user_id', '=', user_id),
        ...
        if not temp:
            ...
            if unit_amount:
                amount_unit = self.on_change_unit_amount(cr, uid, ids,
                                                         r.product_id.id, unit_amount, False, r.product_id.uom_id.id)[
                    'value']['amount']
    ...
    def on_change_user_id(self, cr, uid, ids, user_id, account_id=False, unit_amount=0):
        ...
        if account_id:
            ...
            if temp:
                ...
                if unit_amount:
                    amount_unit = self.on_change_unit_amount(cr, uid, ids,
                                                             r.product_id.id, unit_amount, False,
                                                             r.product_id.uom_id.id)['value']['amount']
    ...
    def _get_invoice_price(self, cr, uid, account, product_id, user_id, qty, context={}):
        for grid in account.user_product_ids:
            if grid.user_id.id == user_id:
                return grid.price
        return super(account_analytic_line, self)._get_invoice_price(cr, uid, account, product_id, user_id, qty,
                                                                     context)
