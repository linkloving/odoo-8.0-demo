...
class specify_partner_terminology(osv.osv_memory):
    ...
    def make_translations(self, cr, uid, ids, name, type, src, value, res_id=0, context=None):
        ...
        existing_trans_ids = trans_obj.search(cr, uid,
                                              [('name', '=', name), ('lang', '=', context_lang), ('type', '=', type),
                                               ('src', '=', src), ('res_id', '=', res_id)])
        if existing_trans_ids:
            trans_obj.write(cr, uid, existing_trans_ids, {'value': value}, context=context)
        else:
            create_id = trans_obj.create(cr, uid,
                                         {'name': name, 'lang': context_lang, 'type': type, 'src': src, 'value': value,
    ...
    def execute(self, cr, uid, ids, context=None):
        ...
        for o in self.browse(cr, uid, ids, context=context):
            ...
            for f_id in fields_obj.browse(cr, uid, field_ids, context=context):
                ...
                self.make_translations(cr, uid, ids, field_ref, 'field', f_id.field_description,
                                       _case_insensitive_replace(f_id.field_description, 'Customer', o.partner),
                                       context=context)
            ...
            for obj in self.pool.models.values():
                for field_name, field_rec in obj._columns.items():
                    if field_rec.help.lower().count('customer'):
                        ...
                        self.make_translations(cr, uid, ids, field_ref, 'help', field_rec.help,
                                               _case_insensitive_replace(field_rec.help, 'Customer', o.partner),
                                               context=context)
            # translate menuitems
            ...
            for m_id in menu_obj.browse(cr, uid, menu_ids, context=context):
                ...
                self.make_translations(cr, uid, ids, menu_ref, 'model', menu_name,
                                       _case_insensitive_replace(menu_name, 'Customer', o.partner), res_id=m_id.id,
                                       context=context)
            ...
            for act_id in act_window_obj.browse(cr, uid, act_window_ids, context=context):
                ...
                self.make_translations(cr, uid, ids, act_ref, 'model', act_id.name,
                                       _case_insensitive_replace(act_id.name, 'Customer', o.partner), res_id=act_id.id,
                                       context=context)
            ...
            for act_id in act_window_obj.browse(cr, uid, act_window_ids, context=context):
                ...
                self.make_translations(cr, uid, ids, act_ref, 'model', act_id.help,
                                       _case_insensitive_replace(act_id.help, 'Customer', o.partner), res_id=act_id.id,
                                       context=context)
        return {}
