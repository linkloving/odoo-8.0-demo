    ...
    def send_mail_test(self, cr, uid, ids, context=None):
        ...
        for wizard in self.browse(cr, uid, ids, context=context):
            ...
            for test_mail in test_emails:
                ...
                mail_mail_obj = Mail.browse(cr, uid, Mail.create(cr, uid, mail_values, context=context),
                                            context=context)
                unsubscribe_url = Mail._get_unsubscribe_url(cr, uid, mail_mail_obj, test_mail, context=context)
                body = tools.append_content_to_html(mailing.body_html, unsubscribe_url, plaintext=False,
                                                    container_tag='p')
