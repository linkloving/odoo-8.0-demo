...
class MassMailingList(osv.Model):
    ...
    def _get_contact_nbr(self, cr, uid, ids, name, arg, context=None):
        ...
        for group in Contacts.read_group(cr, uid, [('list_id', 'in', ids), ('opt_out', '!=', True)], ['list_id'],
                                         ['list_id'], context=context):
...
class MassMailing(osv.Model):
    ...
    def _get_daily_statistics(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for mailing in self.browse(cr, uid, ids, context=context):
            ...
            domain = [('mass_mailing_id', '=', mailing.id), ('opened', '>=', date_begin_str),
                      ('opened', '<=', date_end_str)]
            res[mailing.id]['opened_daily'] = json.dumps(
                self.__get_bar_values(cr, uid, obj, domain, ['opened'], 'opened_count', 'opened:day', date_begin,
                                      context=context))
            domain = [('mass_mailing_id', '=', mailing.id), ('replied', '>=', date_begin_str),
                      ('replied', '<=', date_end_str)]
            res[mailing.id]['replied_daily'] = json.dumps(
                self.__get_bar_values(cr, uid, obj, domain, ['replied'], 'replied_count', 'replied:day', date_begin,
                                      context=context))
        return res
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False,
        ...
        if groupby and groupby[0] == "state":
            ...
            read_group_res = super(MassMailing, self).read_group(cr, uid, domain, fields, groupby, offset=offset,
            ...
            return super(MassMailing, self).read_group(cr, uid, domain, fields, groupby, offset=offset, limit=limit,
    ...
    def action_edit_html(self, cr, uid, ids, context=None):
        ...
        url = '/website_mail/email_designer?model=mail.mass_mailing&res_id=%d&template_model=%s&return_action=%d&enable_editor=1' % (
        ids[0], mail.mailing_model, context['params']['action'])
        return {
    ...
    def get_recipients(self, cr, uid, mailing, context=None):
        ...
        if mailing.contact_ab_pc < 100:
            ...
            if mailing.mass_mailing_campaign_id and mailing.mass_mailing_campaign_id.unique_ab_testing:
                already_mailed = \
                self.pool['mail.mass_mailing.campaign'].get_recipients(cr, uid, [mailing.mass_mailing_campaign_id.id],
                                                                       context=context)[
                    mailing.mass_mailing_campaign_id.id]
            else:
                already_mailed = set([])
