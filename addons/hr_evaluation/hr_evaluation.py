...
class hr_evaluation_plan(osv.Model):
    ...
    _columns = {
        ...
        'month_first': fields.integer('First Appraisal in (months)',
                                      help="This number of months will be used to schedule the first evaluation date of the employee when selecting an evaluation plan. "),
        'month_next': fields.integer('Periodicity of Appraisal (months)',
                                     help="The number of month that depicts the delay between each evaluation of this plan (after the first one)."),
...
class hr_evaluation_plan_phase(osv.Model):
    ...
    _columns = {
        ...
        'company_id': fields.related('plan_id', 'company_id', type='many2one', relation='res.company', string='Company',
        ...
        'mail_feature': fields.boolean('Send mail for this phase',
...
class hr_employee(osv.Model):
    ...
    _columns = {
        ...
        'evaluation_date': fields.date('Next Appraisal Date',
            ...
            for phase in evaluation.plan_id.phase_ids:
                ...
                for child in children:
                    ...
                    int_id = hr_eval_inter_obj.create(cr, uid, {
                        ...
                        'phase_id': phase.id,
                        'deadline': (
                        parser.parse(datetime.now().strftime('%Y-%m-%d')) + relativedelta(months=+1)).strftime(
                            '%Y-%m-%d'),
...
class hr_evaluation_interview(osv.Model):
    ...
    _columns = {
        ...
        'user_to_review_id': fields.related('evaluation_id', 'employee_id', type="many2one", relation="hr.employee",
        ...
        'survey_id': fields.related('phase_id', 'survey_id', string="Appraisal Form", type="many2one",
