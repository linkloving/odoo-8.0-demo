...
class crm_merge_opportunity(osv.osv_memory):
    ...
    _columns = {
        'opportunity_ids': fields.many2many('crm.lead', rel='merge_opportunity_rel', id1='merge_id',
                                            id2='opportunity_id', string='Leads/Opportunities'),
    ...
    def action_merge(self, cr, uid, ids, context=None):
        ...
        merge_id = lead_obj.merge_opportunity(cr, uid, [x.id for x in opportunity2merge_ids], wizard.user_id.id,
                                              wizard.section_id.id, context=context)
    ...
    def on_change_user(self, cr, uid, ids, user_id, section_id, context=None):
        ...
        if user_id:
            if section_id:
                user_in_section = self.pool.get('crm.case.section').search(cr, uid, [('id', '=', section_id), '|',
                                                                                     ('user_id', '=', user_id),
                                                                                     ('member_ids', '=', user_id)],
            ...
            if not user_in_section:
                section_id = False
                section_ids = self.pool.get('crm.case.section').search(cr, uid, ['|', ('user_id', '=', user_id),
                                                                                 ('member_ids', '=', user_id)],
                                                                       context=context)
                if section_ids:
