...
class crm_case_section(osv.Model):
    ...
    def _get_opportunities_data(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for id in ids:
            ...
            res[id]['monthly_open_leads'] = json.dumps(
                self.__get_bar_values(cr, uid, obj, lead_domain, ['create_date'], 'create_date_count', 'create_date',
                                      context=context))
            res[id]['monthly_planned_revenue'] = json.dumps(
                self.__get_bar_values(cr, uid, obj, opp_domain, ['planned_revenue', 'date_deadline'], 'planned_revenue',
                                      'date_deadline', context=context))
        return res
    ...
    _columns = {
        ...
        'use_opportunities': fields.boolean('Opportunities',
                                            help="Check this box to manage opportunities in this sales team."),
    ...
    def _auto_init(self, cr, context=None):
        ...
        return self.pool.get('mail.alias').migrate_to_alias(cr, self._name, self._table,
                                                            super(crm_case_section, self)._auto_init,
                                                            'crm.lead', self._columns['alias_id'], 'name',
                                                            alias_prefix='Lead+', alias_defaults={}, context=context)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        self.pool.get('mail.alias').write(cr, uid, [section.alias_id.id], {'alias_parent_thread_id': section_id,
                                                                           'alias_defaults': {'section_id': section_id,
                                                                                              'type': 'lead'}},
                                          context=context)
        return section_id
