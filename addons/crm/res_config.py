...
class crm_configuration(osv.TransientModel):
    ...
    _defaults = {
        'alias_domain': lambda self, cr, uid, context:
        self.pool['mail.alias']._get_alias_domain(cr, SUPERUSER_ID, [1], None, None)[1],
    }
    ...
    def set_default_alias_prefix(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if not alias_id:
                ...
                alias_id = self.pool['mail.alias'].create(cr, uid, {'alias_name': record.alias_prefix},
                                                          context=create_ctx)
            else:
