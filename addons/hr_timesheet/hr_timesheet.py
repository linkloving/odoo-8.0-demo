...
class hr_employee(osv.osv):
    ...
    _columns = {
        ...
        'uom_id': fields.related('product_id', 'uom_id', type='many2one', relation='product.uom',
                                 string='Unit of Measure', store=True, readonly=True)
    }
    ...
    def _getAnalyticJournal(self, cr, uid, context=None):
        ...
        try:
            ...
            check_right = self.pool.get('account.analytic.journal').search(cr, uid, [('id', '=', res_id)],
                                                                           context=context)
            if check_right:
                return res_id
        except ValueError:
...
class hr_analytic_timesheet(osv.osv):
    ...
    _columns = {
        ...
        'partner_id': fields.related('account_id', 'partner_id', type='many2one', string='Partner',
    ...
    def on_change_unit_amount(self, cr, uid, id, prod_id, unit_amount, company_id, unit=False, journal_id=False,
                              context=None):
        res = {'value': {}}
        if prod_id and unit_amount:
            # find company
            company_id = self.pool.get('res.company')._company_default_get(cr, uid, 'account.analytic.line',
                                                                           context=context)
            r = self.pool.get('account.analytic.line').on_change_unit_amount(cr, uid, id, prod_id, unit_amount,
                                                                             company_id, unit, journal_id,
                                                                             context=context)
            if r:
                res.update(r)
    ...
    def _getAnalyticJournal(self, cr, uid, context=None):
        ...
        if context.get('employee_id'):
            ...
            emp_id = emp_obj.search(cr, uid, [('user_id', '=', context.get('user_id') or uid)], limit=1,
                                    context=context)
        if not emp_id:
            model, action_id = self.pool['ir.model.data'].get_object_reference(cr, uid, 'hr',
