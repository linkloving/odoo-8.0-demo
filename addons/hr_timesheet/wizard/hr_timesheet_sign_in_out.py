...
class hr_so_project(osv.osv_memory):
    ...
    _columns = {
        ...
        'state': fields.related('emp_id', 'state', string='Current Status', type='selection',
    ...
    def _get_empid(self, cr, uid, context=None):
        ...
        if emp_ids:
            for employee in emp_obj.browse(cr, uid, emp_ids, context=context):
                return {'name': employee.name, 'state': employee.state, 'emp_id': emp_ids[0],
                        'server_date': time.strftime('%Y-%m-%d %H:%M:%S')}

    def _get_empid2(self, cr, uid, context=None):
        res = self._get_empid(cr, uid, context=context)
        cr.execute('select name,action from hr_attendance where employee_id=%s order by name desc limit 1',
                   (res['emp_id'],))
    ...
    def _write(self, cr, uid, data, emp_id, context=None):
        ...
        up = timesheet_obj.on_change_unit_amount(cr, uid, False, res['product_id'], hour, False, res['product_uom_id'])[
            'value']
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        if emp_id:
            for employee in emp_obj.browse(cr, uid, emp_id, context=context):
                res.update({'name': employee.name, 'state': employee.state, 'emp_id': emp_id[0],
                            'server_date': time.strftime('%Y-%m-%d %H:%M:%S')})
        return res
