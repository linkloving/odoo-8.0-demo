...
class partner_vat_intra(osv.osv_memory):
    ...
    def _get_europe_country(self, cursor, user, context=None):
        return self.pool.get('res.country').search(cursor, user, [('code', 'in',
                                                                   ['AT', 'BG', 'CY', 'CZ', 'DK', 'EE', 'FI', 'FR',
    ...
    '''
        ...
        'period_ids': fields.many2many('account.period', 'account_period_rel', 'acc_id', 'period_id', 'Period (s)',
        ...
        'no_vat': fields.text('Partner With No VAT', readonly=True,
    ...
    def _get_tax_code(self, cr, uid, context=None):
        ...
        tax_code_ids = obj_tax_code.search(cr, uid, [('company_id', '=', company_id), ('parent_id', '=', False)],
                                           context=context)
    ...
    def _get_datas(self, cr, uid, ids, context=None):
        ...
        for row in cr.dictfetchall():
            ...
            row['intra_code'] == '46L' and 'L' or (row['intra_code'] == '46T' and 'T' or ''))
        ...
        xmldict.update(
            {'dnum': dnum, 'clientnbr': str(seq), 'amountsum': round(amount_sum, 2), 'partner_wo_vat': p_count})
        return xmldict
...
<ns2:IntraConsignment xmlns="http://www.minfin.fgov.be/InputCommon" xmlns:ns2="http://www.minfin.fgov.be/IntraConsignment" IntraListingsNbr="1">
    <ns2:Representative>
        ...
        for client in xml_data['clientlist']:
            if not client['vatnum']:
                raise osv.except_osv(_('Insufficient Data!'),
                                     _('No vat number defined for %s.') % client['partner_name'])
        ...
        model_data_ids = mod_obj.search(cursor, user,
                                        [('model', '=', 'ir.ui.view'), ('name', '=', 'view_vat_intra_save')],
                                        context=context)
