...
class calendar_attendee(osv.Model):
    ...
    def _send_mail_to_attendees(self, cr, uid, ids, email_from=tools.config.get('email_from', False),
        ...
        if self.pool['ir.config_parameter'].get_param(cr, uid, 'calendar.block_mail', default=False) or context.get(
        ...
        local_context.update({
            'color': color,
            'action_id':
                self.pool['ir.actions.act_window'].search(cr, uid, [('view_id', '=', act_id)], context=context)[0],
            'dbname': cr.dbname,
    ...
    def do_accept(self, cr, uid, ids, context=None, *args):
        ...
        for attendee in self.browse(cr, uid, ids, context=context):
            meeting_obj.message_post(cr, uid, attendee.event_id.id,
                                     body=_(("%s has accepted invitation") % (attendee.cn)),
    ...
    def do_decline(self, cr, uid, ids, context=None, *args):
        ...
        for attendee in self.browse(cr, uid, ids, context=context):
            meeting_obj.message_post(cr, uid, attendee.event_id.id,
                                     body=_(("%s has declined invitation") % (attendee.cn)),
                                     subtype="calendar.subtype_invitation", context=context)
        return res
...
class calendar_alarm_manager(osv.AbstractModel):
    ...
    def do_check_alarm_for_one_date(self, cr, uid, one_date, event, event_maxdelta, in_the_next_X_seconds, after=False,
                                    notif=True, mail=True, missing=False, context=None):
            ...
            for alarm in event.alarm_ids:
                if alarm.type in alarm_type and \
                        ...
                        (not after or one_date - timedelta(
                            minutes=alarm.duration_minutes) > openerp.fields.Datetime.from_string(after)):
                    alert = {
                        'alarm_id': alarm.id,
                        'event_id': event.id,
    ...
    def get_next_mail(self, cr, uid, context=None):
        ...
        try:
            cron = self.pool['ir.model.data'].get_object(cr, uid, 'calendar', 'ir_cron_scheduler_alarm',
                                                         context=context)
        except ValueError:
        ...
        for curEvent in self.pool.get('calendar.event').browse(cr, uid, all_events.keys(), context=context):
            ...
            if curEvent.recurrency:
                at_least_one = False
                last_found = False
                for one_date in self.pool.get('calendar.event').get_recurrent_date_by_event(cr, uid, curEvent,
                                                                                            context=context):
                    in_date_format = one_date.replace(tzinfo=None)
                    last_found = self.do_check_alarm_for_one_date(cr, uid, in_date_format, curEvent, max_delta, 0,
                                                                  after=last_notif_mail, notif=False, missing=True,
                                                                  context=context)
    ...
    def get_next_notif(self, cr, uid, context=None):
        ...
        partner = self.pool['res.users'].read(cr, SUPERUSER_ID, uid, ['partner_id', 'calendar_last_notif_ack'],
                                              context=context)
        all_notif = []
        ...
        all_events = self.get_next_potential_limit_alarm(cr, uid, ajax_check_every_seconds,
                                                         partner_id=partner['partner_id'][0], mail=False,
                                                         context=context)
        ...
        for event in all_events:  # .values()
            ...
            if curEvent.recurrency:
                ...
                for one_date in self.pool.get("calendar.event").get_recurrent_date_by_event(cr, uid, curEvent,
                                                                                            context=context):
                    in_date_format = one_date.replace(tzinfo=None)
                    LastFound = self.do_check_alarm_for_one_date(cr, uid, in_date_format, curEvent, max_delta,
                                                                 ajax_check_every_seconds,
                                                                 after=partner['calendar_last_notif_ack'], mail=False,
                                                                 context=context)
                    if LastFound:
...
class calendar_alarm(osv.Model):
    ...
    _columns = {
        ...
        'interval': fields.selection([('minutes', 'Minutes'), ('hours', 'Hours'), ('days', 'Days')], 'Unit',
                                     required=True),
...
class ir_values(osv.Model):
    ...
    def set(self, cr, uid, key, key2, name, models, value, replace=True, isobject=False, meta=False,
    ...
    def get(self, cr, uid, key, key2, models, meta=False, context=None, res_id_req=False, without_user=True,
            key2_req=True):
        if context is None:
            context = {}
        new_model = []
...
class calendar_event(osv.Model):
    ...
    def get_recurrent_date_by_event(self, cr, uid, event, context=None):
        ...
        startdate = pytz.UTC.localize(
            datetime.strptime(event.start, DEFAULT_SERVER_DATETIME_FORMAT))  # Add "+hh:mm" timezone
        if not startdate:
        ...
        ids_depending = self.search(cr, uid, [('recurrent_id', '=', event.id), '|', ('active', '=', False),
    ...
    def _get_recurrency_end_date(self, cr, uid, id, context=None):
        data = self.read(cr, uid, id, ['final_date', 'recurrency', 'rrule_type', 'count', 'end_type', 'stop'],
                         context=context)
    ...
    def _get_display_time(self, cr, uid, start, stop, zduration, zallday, context=None):
        ...
        date = fields.datetime.context_timestamp(cr, uid,
                                                 datetime.strptime(start, tools.DEFAULT_SERVER_DATETIME_FORMAT),
                                                 context=context)
        date_deadline = fields.datetime.context_timestamp(cr, uid,
                                                          datetime.strptime(stop, tools.DEFAULT_SERVER_DATETIME_FORMAT),
                                                          context=context)
        ...
        if zallday:
            ...
            time = _("%s at %s To\n %s at %s (%s)") % (
            event_date, display_time, date_deadline.strftime(format_date), date_deadline.strftime(format_time), tz)
        return time
        ...
        for meeting in self.browse(cr, uid, ids, context=context):
            ...
            for field in fields:
                if field == 'is_attendee':
                    ...
                    meeting_data[field] = self._get_display_time(cr, uid, meeting.start, meeting.stop, meeting.duration,
                                                                 meeting.allday, context=context)
        ...
        if values.get('start_datetime') or values.get('start_date') or values.get('start') \
            ...
            if allday and (values.get('stop_date') or values.get('start_date')):
                ...
                if stop_datetime and start_datetime:
                    diff = openerp.fields.Datetime.from_string(stop_datetime) - openerp.fields.Datetime.from_string(
                        start_datetime)
            if diff:
    ...
    _columns = {
        ...
        'attendee_status': fields.function(_compute, string='Attendee Status', type="selection",
                                           selection=calendar_attendee.STATE_SELECTION, multi='attendee'),
        ...
        'location': fields.char('Location', help="Location of Event", track_visibility='onchange',
                                states={'done': [('readonly', True)]}),
        'show_as': fields.selection([('free', 'Free'), ('busy', 'Busy')], 'Show Time as',
                                    states={'done': [('readonly', True)]}),

        # RECURRENCE FIELD
        'rrule': fields.function(_get_rulestring, type='char', fnct_inv=_set_rulestring, store=True,
                                 string='Recurrent Rule'),
        'rrule_type': fields.selection(
            [('daily', 'Day(s)'), ('weekly', 'Week(s)'), ('monthly', 'Month(s)'), ('yearly', 'Year(s)')], 'Recurrency',
            states={'done': [('readonly', True)]}, help="Let the event automatically repeat at that interval"),
        ...
        'color_partner_id': fields.related('user_id', 'partner_id', 'id', type="integer", string="colorize",
                                           store=False),  # Color of creator
        ...
        'partner_ids': fields.many2many('res.partner', 'calendar_event_res_partner_rel', string='Attendees',
                                        states={'done': [('readonly', True)]}),
        'alarm_ids': fields.many2many('calendar.alarm', 'calendar_alarm_calendar_event_rel', string='Reminders',
    ...
    _constraints = [
        (_check_closing_date, 'Error ! End date cannot be set before start date.',
         ['start_datetime', 'stop_datetime', 'start_date', 'stop_date'])
    ]
    ...
    def onchange_dates(self, cr, uid, ids, fromtype, start=False, end=False, checkallday=False, allday=False,
                       context=None):
    ...
    def create_attendees(self, cr, uid, ids, context=None):
        ...
        for event in self.browse(cr, uid, ids, context):
            ...
            for partner in event.partner_ids:
                ...
                if not current_user.email or current_user.email != partner.email:
                    ...
                    if not context.get('no_email'):
                        if self.pool['calendar.attendee']._send_mail_to_attendees(cr, uid, att_id, email_from=mail_from,
                                                                                  context=context):
                            self.message_post(cr, uid, event.id,
                                              body=_("An invitation email has been sent to attendee %s") % (
                                              partner.name,), subtype="calendar.subtype_invitation", context=context)
            ...
            partner_ids_to_remove = map(lambda x: x,
                                        set(all_part_attendee_ids + new_att_partner_ids) - set(all_partner_ids))

            attendee_ids_to_remove = []

            if partner_ids_to_remove:
                attendee_ids_to_remove = self.pool["calendar.attendee"].search(cr, uid, [
                    ('partner_id.id', 'in', partner_ids_to_remove), ('event_id.id', '=', event.id)], context=context)
    ...
    def get_recurrent_ids(self, cr, uid, event_id, domain, order=None, context=None):
        ...
        if order_fields:
            ...
            sort_params = uniq(
                [comp if comp not in ['start', 'start_date', 'start_datetime'] else 'sort_start' for comp in
                 sort_params])
            sort_params = uniq(
                [comp if comp not in ['-start', '-start_date', '-start_datetime'] else '-sort_start' for comp in
                 sort_params])
    ...
    def message_get_subscription_data(self, cr, uid, ids, user_pid=None, context=None):
        ...
        for virtual_id in ids:
            ...
            result = super(calendar_event, self).message_get_subscription_data(cr, uid, [real_id], user_pid=None,
                                                                               context=context)
    ...
    def message_post(self, cr, uid, thread_id, body='', subject=None, type='notification', subtype=None,
        ...
        return super(calendar_event, self).message_post(cr, uid, thread_id, body=body, subject=subject, type=type,
                                                        subtype=subtype, parent_id=parent_id, attachments=attachments,
                                                        context=context, **kwargs)

    def message_subscribe(self, cr, uid, ids, partner_ids, subtype_ids=None, context=None):
        return super(calendar_event, self).message_subscribe(cr, uid, get_real_ids(ids), partner_ids,
    ...
    def do_sendmail(self, cr, uid, ids, context=None):
        for event in self.browse(cr, uid, ids, context):
            ...
            if current_user.email:
                if self.pool['calendar.attendee']._send_mail_to_attendees(cr, uid,
                                                                          [att.id for att in event.attendee_ids],
                                                                          email_from=current_user.email,
                                                                          context=context):
                    self.message_post(cr, uid, event.id, body=_("An invitation email has been sent to attendee(s)"),
                                      subtype="calendar.subtype_invitation", context=context)
        return
    ...
    def search(self, cr, uid, args, offset=0, limit=0, order=None, context=None, count=False):
        ...
        if not context.get('virtual_id', True):
            return super(calendar_event, self).search(cr, uid, new_args, offset=offset, limit=limit, order=order,
                                                      count=count, context=context)

        # offset, limit, order and count must be treated separately as we may need to deal with virtual ids
        res = super(calendar_event, self).search(cr, uid, new_args, offset=0, limit=0, order=None, context=context,
                                                 count=False)
    ...
    def _name_search(self, cr, user, name='', args=None, operator='ilike', context=None, limit=100, name_get_uid=None):
        ...
        return super(calendar_event, self)._name_search(cr, user, name=name, args=args, operator=operator,
                                                        context=context, limit=limit, name_get_uid=name_get_uid)
            ...
            if (values.get('start_date') or values.get('start_datetime')) and values.get('active', True):
                for the_id in real_ids + new_ids:
                    if attendees_create:
                        ...
                        mail_to_ids = list(
                            set(attendees_create['old_attendee_ids']) - set(attendees_create['removed_attendee_ids']))
                    else:
                        mail_to_ids = [att.id for att in self.browse(cr, uid, the_id, context=context).attendee_ids]

                    if mail_to_ids:
                        current_user = self.pool['res.users'].browse(cr, uid, uid, context=context)
                        if self.pool['calendar.attendee']._send_mail_to_attendees(cr, uid, mail_to_ids,
                                                                                  template_xmlid='calendar_template_meeting_changedate',
                                                                                  email_from=current_user.email,
                                                                                  context=context):
                            self.message_post(cr, uid, the_id, body=_(
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        result = []
        real_data = super(calendar_event, self).read(cr, uid, [real_id for calendar_id, real_id in select],
        ...
        for calendar_id, real_id in select:
            ...
            if not isinstance(ls, (basestring, int, long)) and len(ls) >= 2:
                ...
                if 'display_time' in fields:
                    res['display_time'] = self._get_display_time(cr, uid, ls[1], ls[2], res['duration'], res['allday'],
                                                                 context=context)
...
class mail_message(osv.Model):
    ...
    def search(self, cr, uid, args, offset=0, limit=0, order=None, context=None, count=False):
        ...
        return super(mail_message, self).search(cr, uid, args, offset=offset, limit=limit, order=order, context=context,
                                                count=count)
    ...
    def _find_allowed_model_wise(self, cr, uid, doc_model, doc_dict, context=None):
        ...
        if doc_model == 'calendar.event':
            ...
            for virtual_id in self.pool[doc_model].get_recurrent_ids(cr, uid, doc_dict.keys(), [], order=order,
                                                                     context=context):
...
class ir_attachment(osv.Model):
    ...
    def search(self, cr, uid, args, offset=0, limit=0, order=None, context=None, count=False):
        ...
        for index in range(len(args)):
            if args[index][0] == "res_id" and isinstance(args[index][2], basestring):
                args[index] = (args[index][0], args[index][1], get_real_ids(args[index][2]))
        return super(ir_attachment, self).search(cr, uid, args, offset=offset, limit=limit, order=order,
