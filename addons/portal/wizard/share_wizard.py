...
class share_wizard_portal(osv.TransientModel):
    ...
    _columns = {
        'user_ids': fields.many2many('res.users', 'share_wizard_res_user_rel', 'share_id', 'user_id', 'Existing users',
                                     domain=[('share', '=', True)]),
        'group_ids': fields.many2many('res.groups', 'share_wizard_res_group_rel', 'share_id', 'group_id',
                                      'Existing groups', domain=[('share', '=', False)]),
    }
    ...
    def _create_or_get_submenu_named(self, cr, uid, parent_menu_id, menu_name, context=None):
        ...
        if not parent_menu_id and context.get('group_id'):
            ...
            parent_menu_ids = Menus.search(cr, SUPERUSER_ID,
                                           [('groups_id', 'in', [context.get('group_id')]), ('parent_id', '=', False)],
                                           limit=1, context=cxt)
    ...
    def _sharing_root_menu_id(self, cr, uid, portal, context=None):
        ...
        if parent_menu_id:
            child_menu_id = self._create_or_get_submenu_named(cr, uid, parent_menu_id, SHARED_DOCS_CHILD_MENU,
                                                              context=context)
