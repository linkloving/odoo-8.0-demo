...
class account_invoice(osv.osv):
    ...
    def onchange_partner_id(self, cr, uid, ids, type, partner_id,
        ...
        if partner_id:
            if (type == 'out_invoice'):
                reference_type = self.pool.get('res.partner').browse(cr, uid, partner_id,
                                                                     context=context).out_inv_comm_type
                if reference_type:
                    reference = \
                    self.generate_bbacomm(cr, uid, ids, type, reference_type, partner_id, '', context=context)['value'][
                        'reference']
        res_update = {
                    ...
                    if not self.check_bbacomm(reference):
                        ...
                        if (len(partner_ref_nr) < 3) or (len(partner_ref_nr) > 7):
                            ...
                            seq_ids = self.search(cr, uid,
                                                  [('type', '=', 'out_invoice'), ('reference_type', '=', 'bba'),
                                                   ('reference', 'like',
                                                    '+++%s/%s/%%' % (partner_ref_nr[:3], partner_ref_nr[3:]))],
                                                  order='reference')
                            if seq_ids:
                                prev_seq = int(self.browse(cr, uid, seq_ids[-1]).reference[12:15])
                                if prev_seq < 999:
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('type') == 'out_invoice' and not reference_type:
            ...
            if reference_type == 'bba':
                reference = \
                self.generate_bbacomm(cr, uid, [], vals['type'], reference_type, vals['partner_id'], '', context={})[
                    'value']['reference']
            vals.update({
                'reference_type': reference_type or 'none',
                'reference': reference,
