...
class AdyenController(http.Controller):
    ...
    def adyen_return(self, **post):
        ...
        if post.get('authResult') not in ['CANCELLED']:
            request.registry['payment.transaction'].form_feedback(request.cr, SUPERUSER_ID, post, 'adyen',
    ...
    def adyen_notification(self, **post):
        tx_id = post.get('merchantReference') and request.registry['payment.transaction'].search(request.cr,
                                                                                                 SUPERUSER_ID, [(
                                                                                                                'reference',
                                                                                                                'in', [
                                                                                                                    post.get(
        ...
        if post.get('eventCode') in ['AUTHORISATION'] and tx_id:
            tx = request.registry['payment.transaction'].browse(request.cr, SUPERUSER_ID, tx_id,
                                                                context=request.context)
