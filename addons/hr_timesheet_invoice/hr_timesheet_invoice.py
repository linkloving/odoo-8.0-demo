...
class account_analytic_account(osv.osv):
    ...
    def on_change_partner_id(self, cr, uid, ids, partner_id, name, context=None):
        res = super(account_analytic_account, self).on_change_partner_id(cr, uid, ids, partner_id, name,
                                                                         context=context)
        if partner_id:
...
class account_analytic_line(osv.osv):
    ...
    def _get_invoice_price(self, cr, uid, account, product_id, user_id, qty, context={}):
        ...
        if account.pricelist_id:
            ...
            price = \
            pro_price_obj.price_get(cr, uid, [pl], product_id, qty or 1.0, account.partner_id.id, context=context)[pl]
        else:
            price = 0.0
        return price
    ...
    def _prepare_cost_invoice_line(self, cr, uid, invoice_id, product_id, uom, user_id,
        ...
        if product_id:
            ...
            tax = self.pool['account.fiscal.position'].map_tax(cr, uid, account.partner_id.property_account_position,
        ...
        for (key_id, company_id, currency_id), analytic_lines in invoice_grouping.items():
            ...
            if (not partner) or not (currency_id):
                raise osv.except_osv(_('Error!'), _(
                    'Contract incomplete. Please fill in the Customer and Pricelist fields for %s.') % (account.name))

            curr_invoice = self._prepare_cost_invoice(cr, uid, partner, company_id, currency_id, analytic_lines,
                                                      context=context)
            ...
            for analytic_line in analytic_lines:
                ...
                analytic_line = analytic_line_obj.browse(cr, uid, [line.id for line in analytic_line],
            ...
            for (product_id, uom, user_id, factor_id, account,
                 journal_type), lines_to_invoice in invoice_lines_grouping.items():
        ...
        if inv.type == 'in_invoice':
            ...
            for il in iml:
                if il['account_analytic_id']:
                    ...
                    to_invoice = \
                    obj_analytic_account.read(cr, uid, [il['account_analytic_id']], ['to_invoice'], context=context)[0][
                        'to_invoice']
                    if to_invoice:
...
class account_move_line(osv.osv):
    ...
    def create_analytic_lines(self, cr, uid, ids, context=None):
        ...
        for move_line in self.browse(cr, uid, ids, context=context):
            ...
            invoice_id = move_line.invoice and move_line.invoice.type in (
            'out_invoice', 'out_refund') and move_line.invoice.id or False
