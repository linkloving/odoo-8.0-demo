...
class survey_survey(osv.Model):
    ...
    def filter_input_ids(self, cr, uid, survey, filters, finished=False, context=None):
        ...
        if filters:
            ...
            filtered_input_ids = [input.user_input_id.id for input in
                                  input_line_obj.browse(cr, uid, line_ids, context=context)]
        else:
        ...
        if filters:
            ...
            for filter in filters:
                ...
                filter_display_data.append(
                    {'question_text': question.question, 'labels': [label.value for label in labels]})
    ...
    def prepare_result(self, cr, uid, question, current_filters=None, context=None):
        ...
        if question.type in ['simple_choice', 'multiple_choice']:
            answers = {}
            comments = []
            [answers.update({label.id: {'text': label.value, 'count': 0, 'answer_id': label.id}}) for label in
             question.labels_ids]
...
class survey_question(osv.Model):
    ...
    def validate_datetime(self, cr, uid, question, post, answer_tag, context=None):
        ...
        if answer and question.validation_required:
            ...
            try:
                dateanswer = datetime.datetime.strptime(answer, DF)
                if not (datetime.datetime.strptime(question.validation_min_date,
                                                   DF) <= dateanswer <= datetime.datetime.strptime(
    ...
    def validate_simple_choice(self, cr, uid, question, post, answer_tag, context=None):
        ...
        if question.constr_mandatory and answer_tag in post and post[
            answer_tag] == "-1" and question.comment_count_as_answer and comment_tag in post and not post[
            comment_tag].strip():
            errors.update({answer_tag: question.constr_error_msg})
        return errors
...
class survey_label(osv.Model):
    ...
    _columns = {
        ...
        'quizz_mark': fields.float('Score for this answer',
                                   help="A positive score indicates a correct answer; a negative or null score indicates a wrong answer"),
    }
    _defaults = {
        'sequence': 10,
    }
    _constraints = [
        (_check_question_not_empty, "A label must be attached to one and only one question",
        ...
        if question.matrix_subtype == 'simple':
            for row in question.labels_ids_2:
                ...
                if a_tag in ca:
                    no_answers = False
                    vals.update(
                        {'answer_type': 'suggestion', 'value_suggested': ca[a_tag], 'value_suggested_row': row.id})
                    self.create(cr, uid, vals, context=context)
