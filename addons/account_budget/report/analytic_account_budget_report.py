...
class analytic_account_budget_report(report_sxw.rml_parse):
    ...
    def funct(self, object, form, ids=None, done=None, level=1):
        ...
        for account_id in accounts:
            ...
            self.cr.execute('SELECT DISTINCT(crossovered_budget_id) FROM crossovered_budget_lines WHERE id =ANY(%s)',
                            (b_line_ids,))
            ...
            for i in range(0, len(budget_ids)):
                ...
                line_ids = c_b_lines_obj.search(self.cr, self.uid, [('id', 'in', b_line_ids),
                    ...
                    if line.id in b_line_ids:
                        ...
                        if line.general_budget_id.id in done_budget:
                            for record in result:
                                if record['b_id'] == line.general_budget_id.id and record[
