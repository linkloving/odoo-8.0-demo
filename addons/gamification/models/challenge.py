...
class gamification_challenge(osv.Model):
    ...
    def _get_report_template(self, cr, uid, context=None):
        try:
            return \
            self.pool.get('ir.model.data').get_object_reference(cr, uid, 'gamification', 'simple_report_template')[1]
        except ValueError:
            return False
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('report_message_frequency', 'never') != 'never':
            ...
            for challenge in self.browse(cr, uid, ids, context=context):
                self.message_subscribe(cr, uid, [challenge.id], [user.partner_id.id for user in challenge.user_ids],
                                       context=context)
        ...
        if vals.get('state') == 'inprogress':
            ...
            # resetting progress
            if self.pool.get('gamification.goal').search(cr, uid,
                                                         [('challenge_id', 'in', ids), ('state', '=', 'inprogress')],
                                                         context=context):
    ...
    def quick_update(self, cr, uid, challenge_id, context=None):
        ...
        goal_ids = self.pool.get('gamification.goal').search(cr, uid, [('challenge_id', '=', challenge_id)],
                                                             context=context)
    ...
    def action_check(self, cr, uid, ids, context=None):
        ...
        goal_ids = goal_obj.search(cr, uid, [('challenge_id', 'in', ids), ('state', '=', 'inprogress')],
                                   context=context)
    ...
    def _generate_goals_from_challenge(self, cr, uid, ids, context=None):
        ...
        for challenge in self.browse(cr, uid, ids, context=context):
            ...
            for line in challenge.line_ids:
                ...
                user_without_goal_ids = list(
                    set(participant_user_ids) - set([user['user_id'] for user in user_with_goal_ids]))
                user_squating_challenge_ids = list(
                    set([user['user_id'] for user in user_with_goal_ids]) - set(participant_user_ids))
                if user_squating_challenge_ids:
                    # users that used to match the challenge 
                    goal_to_remove_ids = goal_obj.search(cr, uid, [('challenge_id', '=', challenge.id),
                                                                   ('user_id', 'in', user_squating_challenge_ids)],
                                                         context=context)
    ...
    def _get_serialized_challenge_lines(self, cr, uid, challenge, user_id=False, restrict_goal_ids=False,
                                        restrict_top=False, context=None):
    ...
    def report_progress(self, cr, uid, challenge, context=None, users=False, subset_goal_ids=False):
        ...
        if challenge.visibility_mode == 'ranking':
            lines_boards = self._get_serialized_challenge_lines(cr, uid, challenge, user_id=False,
                                                                restrict_goal_ids=subset_goal_ids, restrict_top=False,
                                                                context=context)

            ctx.update({'challenge_lines': lines_boards})
            body_html = temp_obj.render_template(cr, uid, challenge.report_template_id.body_html,
                                                 'gamification.challenge', challenge.id, context=ctx)
            ...
            for user in users or challenge.user_ids:
                goals = self._get_serialized_challenge_lines(cr, uid, challenge, user.id,
                                                             restrict_goal_ids=subset_goal_ids, context=context)
                if not goals:
                    continue

                ctx.update({'challenge_lines': goals})
                body_html = temp_obj.render_template(cr, user.id, challenge.report_template_id.body_html,
                                                     'gamification.challenge', challenge.id, context=ctx)
    ...
    def _accept_challenge(self, cr, uid, user_id, challenge_ids, context=None):
        ...
        self.write(cr, SUPERUSER_ID, challenge_ids, {'invited_user_ids': [(3, user_id)], 'user_ids': [(4, user_id)]},
                   context=context)
        ...
        for challenge in self.browse(cr, uid, ids, context=context):
            ...
            if challenge_ended:
                ...
                if rewarded_users:
                    ...
                    challenge.reward_id.name, ", ".join([name for (user_id, name) in user_names])))
                else:
                    message_body += _(
                ...
                if challenge.reward_first_id:
                    ...
                    if first_user:
                        self.reward_user(cr, uid, first_user.id, challenge.reward_first_id.id, challenge.id,
                                         context=context)
                        message_body += _(
                    ...
                    if second_user and challenge.reward_second_id:
                        self.reward_user(cr, uid, second_user.id, challenge.reward_second_id.id, challenge.id,
                                         context=context)
                        message_body += "<br/> 2. %s - %s" % (second_user.name, challenge.reward_second_id.name)
                    if third_user and challenge.reward_third_id:
                        self.reward_user(cr, uid, third_user.id, challenge.reward_second_id.id, challenge.id,
                                         context=context)
    ...
    def reward_user(self, cr, uid, user_id, badge_id, challenge_id=False, context=None):
        ...
        user_badge_id = badge_user_obj.create(cr, uid,
                                              {'user_id': user_id, 'badge_id': badge_id, 'challenge_id': challenge_id},
                                              context=context)
...
class gamification_challenge_line(osv.Model):
    ...
    _columns = {
        ...
        'definition_monetary': fields.related('definition_id', 'monetary', type="boolean", readonly=True,
                                              string="Monetary"),
        'definition_full_suffix': fields.related('definition_id', 'full_suffix', type="char", readonly=True,
                                                 string="Suffix"),
    }
