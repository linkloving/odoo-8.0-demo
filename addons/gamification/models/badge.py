...
class gamification_badge_user(osv.Model):
    ...
    def _send_badge(self, cr, uid, ids, context=None):
        ...
        template_id = self.pool['ir.model.data'].get_object(cr, uid, 'gamification', 'email_template_badge_received',
                                                            context)
        for badge_user in self.browse(cr, uid, ids, context=context):
            body_html = temp_obj.render_template(cr, uid, template_id.body_html, 'gamification.badge.user',
                                                 badge_user.id, context=context)
...
class gamification_badge(osv.Model):
    ...
    def _get_badge_user_stats(self, cr, uid, ids, name, args, context=None):
        ...
        for bid in ids:
            result[bid] = {
                'stat_my': badge_user_obj.search(cr, uid, [('badge_id', '=', bid), ('user_id', '=', uid)],
                                                 context=context, count=True),
                'stat_this_month': badge_user_obj.search(cr, uid, [('badge_id', '=', bid),
                                                                   ('create_date', '>=', first_month_day)],
                                                         context=context, count=True),
                'stat_my_this_month': badge_user_obj.search(cr, uid, [('badge_id', '=', bid), ('user_id', '=', uid),
                                                                      ('create_date', '>=', first_month_day)],
                                                            context=context, count=True),
                'stat_my_monthly_sending': badge_user_obj.search(cr, uid,
                                                                 [('badge_id', '=', bid), ('create_uid', '=', uid),
                                                                  ('create_date', '>=', first_month_day)],
                                                                 context=context, count=True)
            }
        return result
    ...
    def _can_grant_badge(self, cr, uid, badge_id, context=None):
        ...
        if badge.rule_auth == 'nobody':
            ...
            all_user_badges = self.pool.get('gamification.badge.user').search(cr, uid, [('user_id', '=', uid)],
                                                                              context=context)
