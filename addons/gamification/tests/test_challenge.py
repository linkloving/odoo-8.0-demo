...
class test_challenge(common.TransactionCase):
    def setUp(self):
        ...
        self.challenge_base_id = self.data_obj.get_object_reference(cr, uid, 'gamification', 'challenge_base_discover')[
            1]
        self.definition_timezone_id = \
        self.data_obj.get_object_reference(cr, uid, 'gamification', 'definition_base_timezone')[1]
    ...
    def test_10_reach_challenge(self):
        ...
        goal_ids = self.goal_obj.search(cr, uid,
                                        [('challenge_id', '=', self.challenge_base_id), ('state', '!=', 'draft')],
                                        context=context)
        self.assertEqual(len(goal_ids), len(line_ids) * len(challenge_user_ids),
        ...
        goal_ids = self.goal_obj.search(cr, uid, [('user_id', '=', self.demo_user_id),
                                                  ('definition_id', '=', self.definition_timezone_id)], context=context)

        self.goal_obj.update(cr, uid, goal_ids, context=context)
        reached_goal_ids = self.goal_obj.search(cr, uid, [('id', 'in', goal_ids), ('state', '=', 'reached')],
                                                context=context)
        ...
        self.challenge_obj.write(cr, uid, self.challenge_base_id,
                                 {'reward_first_id': self.badge_id, 'reward_second_id': self.badge_id}, context=context)
        self.challenge_obj.write(cr, uid, self.challenge_base_id, {'state': 'done'}, context=context)

        badge_ids = self.badge_user_obj.search(cr, uid,
                                               [('badge_id', '=', self.badge_id), ('user_id', '=', self.demo_user_id)])
