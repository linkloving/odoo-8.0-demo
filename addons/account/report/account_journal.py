...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if (data['model'] == 'ir.ui.menu'):
            ...
            self.query_get_clause += obj_move._query_get(self.cr, self.uid, obj='l',
            ...
            self.cr.execute('SELECT period_id, journal_id FROM account_journal_period WHERE id IN %s',
                            (tuple(new_ids),))
    ...
    def lines(self, period_id, journal_id=False):
        ...
        self.cr.execute(
            'update account_journal_period set state=%s where journal_id IN %s and period_id=%s and state=%s',
            ('printed', self.journal_ids, period_id, 'draft'))
        ...
        self.cr.execute(
            'SELECT l.id FROM account_move_line l, account_move am WHERE l.move_id=am.id AND am.state IN %s AND l.period_id=%s AND l.journal_id IN %s ' + self.query_get_clause + ' ORDER BY ' + self.sort_selection + ', l.move_id',
