...
class account_bank_statement(osv.osv):
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('name', '/') == '/':
            ...
            vals['name'] = self._compute_default_statement_name(cr, uid, journal_id,
                                                                context=dict(context, period_id=vals.get('period_id')))
        if 'line_ids' in vals:
    ...
    def _default_journal_id(self, cr, uid, context=None):
        ...
        company_id = self.pool.get('res.company')._company_default_get(cr, uid, 'account.bank.statement',
                                                                       context=context)
        if journal_type:
    ...
    def _compute_default_statement_name(self, cr, uid, journal_id, context=None):
        ...
        period = self.pool.get('account.period').browse(cr, uid, context.get('period_id') or self._get_period(cr, uid,
                                                                                                              context=context),
                                                        context=context)
    ...
    _columns = {
        ...
        'balance_end': fields.function(_end_balance,
                                       store={
                                           'account.bank.statement': (lambda self, cr, uid, ids, c={}: ids,
                                                                      ['line_ids', 'move_line_ids', 'balance_start'],
                                                                      10),
        ...
        'cash_control': fields.related('journal_id', 'cash_control', type='boolean', relation='account.journal',
                                       string='Cash control'),
    ...
    def balance_check(self, cr, uid, st_id, journal_type='bank', context=None):
        ...
        if not ((abs((st.balance_end or 0.0) - st.balance_end_real) < 0.0001) or (
    ...
    def button_confirm_bank(self, cr, uid, ids, context=None):
        ...
        for st in self.browse(cr, uid, ids, context=context):
            ...
            self.message_post(cr, uid, [st.id],
                              body=_('Statement %s confirmed, journal items were created.') % (st.name,),
                              context=context)
        self.link_bank_to_partner(cr, uid, ids, context=context)
        return self.write(cr, uid, ids, {'state': 'confirm', 'closing_date': time.strftime("%Y-%m-%d %H:%M:%S")},
                          context=context)
    ...
    def number_of_lines_reconciled(self, cr, uid, ids, context=None):
        ...
        return bsl_obj.search_count(cr, uid, [('statement_id', 'in', ids), ('journal_entry_id', '!=', False)],
                                    context=context)
        ...
        for statement in self.browse(cr, uid, ids, context=context):
            for st_line in statement.line_ids:
                if st_line.bank_account_id and st_line.partner_id and st_line.bank_account_id.partner_id.id != st_line.partner_id.id:
                    ...
                    bank_vals = \
                    bank_obj.onchange_partner_id(cr, uid, [st_line.bank_account_id.id], st_line.partner_id.id,
...
class account_bank_statement_line(osv.osv):
            ...
            if line.journal_entry_id:
                ...
                for aml in line.journal_entry_id.line_id:
                    if aml.reconcile_id:
                        ...
                        if len(move_lines) >= 2:
                            self.pool.get('account.move.line').reconcile_partial(cr, uid, move_lines, 'auto',
                                                                                 context=context)
        if move_ids:
            account_move_obj.button_cancel(cr, uid, move_ids, context=context)
            account_move_obj.unlink(cr, uid, move_ids, context)

    def get_data_for_reconciliations(self, cr, uid, ids, excluded_ids=None, search_reconciliation_proposition=True,
                                     context=None):
        ...
        for st_line in self.browse(cr, uid, ids, context=context):
            ...
            if search_reconciliation_proposition:
                reconciliation_proposition = self.get_reconciliation_proposition(cr, uid, st_line,
    ...
    def get_reconciliation_proposition(self, cr, uid, st_line, excluded_ids=None, context=None):
        ...
        if st_line.name:
            domain = self._domain_reconciliation_proposition(cr, uid, st_line, excluded_ids=excluded_ids,
                                                             context=context)
            ...
            if match_id and len(match_id) == 1:
                ...
                mv_line = mv_line_pool.prepare_move_lines_for_reconciliation_widget(cr, uid, mv_line_br,
                ...
                if move:
                    ...
                    match_recs = self.get_move_lines_for_reconciliation(cr, uid, st_line, excluded_ids=excluded_ids,
                                                                        limit=2, additional_domain=domain)
        ...
        match_id = self.get_move_lines_for_reconciliation(cr, uid, st_line, excluded_ids=excluded_ids, offset=0,
        ...
        if amount_field == 'amount_currency' and amount < 0:
            ...
            domain += [(amount_field, '>', 0), (amount_field, '<=', (sign * amount))]
        mv_lines = self.get_move_lines_for_reconciliation(cr, uid, st_line, excluded_ids=excluded_ids, limit=5,
    ...
    def get_move_lines_for_reconciliation_by_statement_line_id(self, cr, uid, st_line_id, excluded_ids=None, str=False,
        ...
        return self.get_move_lines_for_reconciliation(cr, uid, st_line, excluded_ids, str, offset, limit, count,
                                                      additional_domain, context=context)

    def _domain_move_lines_for_reconciliation(self, cr, uid, st_line, excluded_ids=None, str=False,
                                              additional_domain=None, context=None):
        if excluded_ids is None:
            excluded_ids = []
        if additional_domain is None:
    ...
    def get_move_lines_for_reconciliation(self, cr, uid, st_line, excluded_ids=None, str=False, offset=0, limit=None,
        ...
        """
        mv_line_pool = self.pool.get('account.move.line')
        domain = self._domain_move_lines_for_reconciliation(cr, uid, st_line, excluded_ids=excluded_ids, str=str,
        ...
        # Or return list of dicts representing the formatted move lines
            ...
            target_currency = st_line.currency_id or st_line.journal_id.currency or st_line.journal_id.company_id.currency_id
            mv_lines = mv_line_pool.prepare_move_lines_for_reconciliation_widget(cr, uid, lines,
                                                                                 target_currency=target_currency,
                                                                                 target_date=st_line.date,
                                                                                 context=context)
        ...
        for mv_line_dict in mv_line_dicts:
            ...
            if st_line_currency.id != company_currency.id:
                ...
                if st_line.currency_id and statement_currency.id == company_currency.id and st_line_currency_rate:
                    debit_at_current_rate = self.pool.get('res.currency').round(cr, uid, company_currency, mv_line_dict[
                    ...
                    debit_at_current_rate = currency_obj.compute(cr, uid, statement_currency.id, company_currency.id,
                                                                 mv_line_dict['debit'] / st_line_currency_rate,
                                                                 context=ctx)
                    credit_at_current_rate = currency_obj.compute(cr, uid, statement_currency.id, company_currency.id,
                ...
                if mv_line_dict.get('counterpart_move_line_id'):
                    ...
                    if mv_line.currency_id and mv_line_dict['currency_id'] == mv_line.currency_id.id:
                        ...
                        amount_unreconciled = currency_obj.compute(cr, uid, company_currency.id,
                                                                   mv_line_dict['currency_id'], mv_line.amount_residual,
                                                                   context=ctx)
    ...
    def _needaction_domain_get(self, cr, uid, context=None):
        ...
        return ['|', ('company_id', '=', False), ('company_id', 'child_of', [user.company_id.id]),
                ('journal_entry_id', '=', False), ('account_id', '=', False)]
    ...
    _columns = {
        ...
        'company_id': fields.related('statement_id', 'company_id', type='many2one', relation='res.company',
                                     string='Company', store=True, readonly=True),
    ...
    _defaults = {
        'name': lambda self, cr, uid, context={}: self.pool.get('ir.sequence').get(cr, uid,
                                                                                   'account.bank.statement.line',
                                                                                   context=context),
        'date': lambda self, cr, uid, context={}: context.get('date', fields.date.context_today(self, cr, uid,
                                                                                                context=context)),
        'sequence': 1,
    }
...
class account_statement_operation_template(osv.osv):
    ...
    _columns = {
        ...
        'amount_type': fields.selection([('fixed', 'Fixed'), ('percentage_of_total', 'Percentage of total amount'),
        ...
        'analytic_account_id': fields.many2one('account.analytic.account', 'Analytic Account', ondelete='set null',
                                               domain=[('type', '!=', 'view'),
