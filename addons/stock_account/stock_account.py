...
class stock_inventory(osv.osv):
    ...
    _columns = {
        'period_id': fields.many2one('account.period', 'Force Valuation Period',
...
class stock_quant(osv.osv):
    ...
    def _account_entry_move(self, cr, uid, quants, move, context=None):
        ...
        if company_to and (move.location_id.usage not in (
        'internal', 'transit') and move.location_dest_id.usage == 'internal' or company_from != company_to):
            ctx = context.copy()
        ...
        if company_from and (move.location_id.usage == 'internal' and move.location_dest_id.usage not in (
        'internal', 'transit') or company_from != company_to):
            ctx = context.copy()
    ...
    def _quant_create(self, cr, uid, qty, move, lot_id=False, owner_id=False, src_package_id=False,
                      dest_package_id=False, force_location_from=False, force_location_to=False, context=None):
        quant_obj = self.pool.get('stock.quant')
        quant = super(stock_quant, self)._quant_create(cr, uid, qty, move, lot_id=lot_id, owner_id=owner_id,
        ...
        if move.product_id.valuation == 'real_time':
            ...
            if float_compare(quant.product_id.uom_id.rounding, 1.0, precision_digits=1) == 0 \
                    and float_compare(quant.qty * quant.cost, quant.qty * cost_rounded,
                                      precision_rounding=curr_rounding) != 0 \
    ...
    def move_quants_write(self, cr, uid, quants, move, location_dest_id, dest_package_id, context=None):
        res = super(stock_quant, self).move_quants_write(cr, uid, quants, move, location_dest_id, dest_package_id,
                                                         context=context)
        if move.product_id.valuation == 'real_time':
...
class stock_move(osv.osv):
    ...
    def _store_average_cost_price(self, cr, uid, move, context=None):
        ...
        product_obj.write(cr, SUPERUSER_ID, [move.product_id.id], {'standard_price': average_valuation_price},
                          context=ctx)
