...
class email_template(osv.osv):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if res.get('model'):
            res['model_id'] = \
            self.pool['ir.model'].search(cr, uid, [('model', '=', res.pop('model'))], context=context)[0]
        return res
    ...
    def render_template_batch(self, cr, uid, template, model, res_ids, context=None, post_process=False):
        ...
        variables = {
            'format_tz': lambda dt, tz=False, format=False, context=context: format_tz(self.pool, cr, uid, dt, tz,
                                                                                       format, context),
            'user': user,
    ...
    _columns = {
        ...
        'body_html': fields.html('Body', translate=True, sanitize=False,
                                 help="Rich-text/HTML version of the message (placeholders may be used here)"),
        ...
        for template in self.browse(cr, uid, ids, context=context):
            try:
                if template.ref_ir_act_window:
                    self.pool.get('ir.actions.act_window').unlink(cr, SUPERUSER_ID, template.ref_ir_act_window.id,
                                                                  context)
    ...
    def onchange_sub_model_object_value_field(self, cr, uid, ids, model_object_field, sub_model_object_field=False,
        ...
        if model_object_field:
            ...
            if field_value.ttype in ['many2one', 'one2many', 'many2many']:
                res_ids = self.pool.get('ir.model').search(cr, uid, [('model', '=', field_value.relation)],
                                                           context=context)
    ...
    def generate_email_batch(self, cr, uid, template_id, res_ids, context=None, fields=None):
        ...
        for template, template_res_ids in templates_to_res_ids.iteritems():
            ...
            if template.report_template:
                for res_id in template_res_ids:
                    attachments = []
                    report_name = self.render_template(cr, uid, template.report_name, template.model, res_id,
                                                       context=ctx)
                    report = report_xml_pool.browse(cr, uid, template.report_template.id, context)
                    report_service = report.report_name

                    if report.report_type in ['qweb-html', 'qweb-pdf']:
                        result, format = self.pool['report'].get_pdf(cr, uid, [res_id], report_service,
                                                                     context=ctx), 'pdf'
                    else:
                        result, format = openerp.report.render_report(cr, uid, [res_id], report_service,
