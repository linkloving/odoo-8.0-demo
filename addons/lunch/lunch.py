...
class lunch_order(osv.Model):
    ...
    def check_day(self, alert):
        ...
        mapping = dict((idx, name) for idx, name in
                       enumerate('days monday tuesday wednesday thursday friday saturday sunday'.split()))
    ...
    def onchange_price(self, cr, uid, ids, order_line_ids, context=None):
        ...
        """
        res = {'value': {'total': 0.0}}
        order_line_ids = self.resolve_o2m_commands_to_record_dicts(cr, uid, "order_line_ids", order_line_ids, ["price"],
                                                                   context=context)
        if order_line_ids:
            tot = 0.0
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        ...
        """
        res = super(lunch_order, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context,
...
class lunch_order_line(osv.Model):
    ...
    _columns = {
        ...
        'supplier': fields.related('product_id', 'supplier', type='many2one', relation='res.partner', string="Supplier",
                                   readonly=True, store=True),
        'user_id': fields.related('order_id', 'user_id', type='many2one', relation='res.users', string='User',
                                  readonly=True, store=True),
...
class lunch_product(osv.Model):
    ...
    _columns = {
        ...
        # TODO: use decimal precision of 'Account', move it from product to decimal_precision
...
class lunch_cashmove(osv.Model):
    ...
    _columns = {
        ...
        # depending on the kind of cashmove, the amount will be positive or negative
