...
class AddAttachment(unohelper.Base, XJobExecutor):
    ...
    def __init__(self, ctx):
        ...
        if docinfo.getUserFieldValue(2) <> "" and docinfo.getUserFieldValue(3) <> "":
            ...
            self.win.addButton('btnOkWithoutInformation', -2, -5, 25, 15, 'OK',
                               actionListenerProc=self.btnOkWithoutInformation_clicked)
        else:
            ...
            ids = self.sock.execute(database, uid, self.password, 'ir.module.module', 'search',
                                    [('name', '=', 'base_report_model'), ('state', '=', 'installed')])
            if not len(ids):
                ...
                res = self.sock.execute(database, uid, self.password, 'base.report.model', 'read', ids,
                                        ['name', 'model_id'])
                models = self.sock.execute(database, uid, self.password, 'ir.model', 'read',
                                           map(lambda x: x['model_id'][0], res), ['model'])
            ...
            self.win.addButton('btnOkWithInformation', -2, -5, 25, 15, 'OK',
                               actionListenerProc=self.btnOkWithInformation_clicked)
    ...
    def btnSearch_clicked(self, oActionEvent):
        ...
        self.aSearchResult = self.sock.execute(database, uid, self.password, self.dModel[modelSelectedItem],
                                               'name_search', self.win.getEditText("txtSearchName"))
    ...
    def doc2pdf(self, strFile):
        ...
        if oDoc:
            ...
            if len(strFilterSubName) > 0:
                oDoc.storeToURL(filename, Array(self._MakePropertyValue("FilterName", strFilterSubName),
                                                self._MakePropertyValue("CompressMode", "1")))
...
if __name__ <> "package" and __name__ == "__main__":
    ...
    g_ImplementationHelper.addImplementation(AddAttachment, "org.openoffice.openerp.report.addattachment",
