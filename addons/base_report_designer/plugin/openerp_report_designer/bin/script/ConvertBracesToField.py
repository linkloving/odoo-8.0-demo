                    ...
                    for reg in regexes:
                        ...
                        if len(res) <> 0:
                            if res[0][0] == "objects":
                                ...
                                sObject = self.getRes(self.sock, docinfo.getUserFieldValue(3),
                                                      res[0][0][res[0][0].find(".") + 1:].replace(".", "/"))
                                r = self.sock.execute(database, uid, self.password, docinfo.getUserFieldValue(3),
                                                      'fields_get')
                                ...
                                for rl in saRepeatInList:
                                    if rl[0] == res[0][:res[0].find(".")]:
                                        obj = rl[1]
                                try:
                                    sObject = self.getRes(self.sock, obj,
                                                          res[0][res[0].find(".") + 1:].replace(".", "/"))
...
if __name__ <> "package":
    ...
    g_ImplementationHelper.addImplementation(ConvertBracesToField, "org.openoffice.openerp.report.convertBF",
