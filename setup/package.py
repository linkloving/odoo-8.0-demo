...
class KVM(object):
    ...
    def ssh(self, cmd):
        l = ['ssh', '-o', 'UserKnownHostsFile=/dev/null', '-o', 'StrictHostKeyChecking=no', '-p', '10022', '-i',
             self.ssh_key, '%s@127.0.0.1' % self.login, cmd]
        system(l)

    def rsync(self, args, options='--delete --exclude .bzrignore'):
        cmd = 'rsync -rt -e "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -p 10022 -i %s" %s %s' % (
        self.ssh_key, options, args)
        system(cmd)
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh(
            'PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Odoo %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh(
            '/cygdrive/c/"Program Files"/"Odoo %s"/server/openerp-server.exe -d mycompany -i base --stop-after-init' % setupversion)
...
def build_tgz(o):
    ...
    system(['mv', glob('%s/dist/odoo-*.tar.gz' % o.build_dir)[0],
            '%s/odoo_%s.%s.tar.gz' % (o.build_dir, version, timestamp)])
...
def build_rpm(o):
    ...
    system(['mv', glob('%s/dist/odoo-*.noarch.rpm' % o.build_dir)[0],
            '%s/odoo_%s.%s.noarch.rpm' % (o.build_dir, version, timestamp)])
...
def test_tgz(o):
    with docker('odoo-%s-src-nightly-tests' % version, o.build_dir, o.pub) as wheezy:
        ...
        wheezy.system(
            'su odoo -s /bin/bash -c "odoo.py --addons-path=/usr/local/lib/python2.7/dist-packages/openerp/addons -d mycompany -i base --stop-after-init"')
        wheezy.system(
            'su odoo -s /bin/bash -c "odoo.py --addons-path=/usr/local/lib/python2.7/dist-packages/openerp/addons -d mycompany &"')
...
def test_deb(o):
    with docker('odoo-%s-debian-nightly-tests' % version, o.build_dir, o.pub) as wheezy:
        ...
        wheezy.system(
            'su odoo -s /bin/bash -c "odoo.py -c /etc/odoo/openerp-server.conf -d mycompany -i base --stop-after-init"')
...
def test_rpm(o):
    with docker('odoo-%s-centos-nightly-tests' % version, o.build_dir, o.pub) as centos7:
        ...
        centos7.system(
            'su odoo -s /bin/bash -c "openerp-server -c /etc/odoo/openerp-server.conf -d mycompany -i base --stop-after-init"')
...
def gen_deb_package(o, published_files):
    ...
    subprocess.call(
        ['gpg', '--default-key', GPGID, '--passphrase', GPGPASSPHRASE, '--yes', '-abs', '--no-tty', '-o', 'Release.gpg',
         'Release'], cwd=os.path.join(o.pub, 'deb'))
...
def options():
    ...
    op.add_option("", "--vm-winxp-python-version", default='2.7',
                  help="Windows Python version installed in the VM (default: %default)")
